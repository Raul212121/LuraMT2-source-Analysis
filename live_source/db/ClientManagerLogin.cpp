...
void CClientManager::QUERY_LOGIN_BY_KEY(CPeer * pkPeer, DWORD dwHandle, TPacketGDLoginByKey * p)
{
	...
	if (pkLoginData->IsLoginKeyExpired())
	{
		sys_log(0, "LOGIN_BY_KEY key is expired login(%s) key(%u)", szLogin, p->dwLoginKey);
		pkPeer->EncodeReturn(HEADER_DG_LOGIN_NOT_EXIST, dwHandle); // TODO: args
		return;
	}
	...
	pkTab->discordUserId = r.discordUserId;

	ClientHandleInfo * info = new ClientHandleInfo(dwHandle);
	info->pAccountTable = pkTab;
	strlcpy(info->ip, p->szIP, sizeof(info->ip));

	sys_log(0, "LOGIN_BY_KEY success %s %lu %s discordUserId(%llu)", r.login, p->dwLoginKey, info->ip, r.discordUserId);
...
void CClientManager::ExpiredLoginKey(const TPacketExpiredLoginKey* packet)
{
	CLoginData* pkLoginData = GetLoginData(packet->dwLoginKey);
	if (pkLoginData == nullptr)
	{
		sys_err("invalid login data by key %u", packet->dwLoginKey);
		return;
	}

	char login[LOGIN_MAX_LEN + 1];
	trim_and_lower(packet->login, login, sizeof(login));

	const TAccountTable& r = pkLoginData->GetAccountRef();
	if (strcasecmp(r.login, login) != 0)
	{
		sys_err("invalid login info by key %u", packet->dwLoginKey);
		return;
	}

	sys_log(0, "expired login key %s %u", login, packet->dwLoginKey);
	pkLoginData->SetLoginKeyExpired(true);
}
