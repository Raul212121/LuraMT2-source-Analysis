	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		else
		{
			snprintf(szQuery, sizeof(szQuery),
					...
					"FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','UPGRADE_INVENTORY','POTIONS_INVENTORY','BONUS_INVENTORY','CHEST_INVENTORY', 'SWITCHBOT', 'BELT_INVENTORY'))",
					GetTablePostfix(), pTab->id);
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','UPGRADE_INVENTORY','POTIONS_INVENTORY','BONUS_INVENTORY','CHEST_INVENTORY', 'SWITCHBOT', 'BELT_INVENTORY'))", GetTablePostfix(), pi->player_id);
		delete CDBManager::instance().DirectQuery(queryStr);
