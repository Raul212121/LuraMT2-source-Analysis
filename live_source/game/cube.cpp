...
bool Cube_make (LPCHARACTER ch)
{
	...
	if (npc->GetRaceNum() == 9009)
	{
		const int flag = quest::CQuestManager::instance().GetEventFlag("eveniment_pescuit");
		if (flag == 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Evenimentul de pescuit este dezactivat."));
			return false;
		}
	}
    ...
    for (int i=0; i<CUBE_MAX_NUM; ++i)
    {
        ...
        if (items[i]->GetType() == ITEM_WEAPON || items[i]->GetType() == ITEM_ARMOR)
        {
            ...
            for (size_t j = 0; j < cube_proto->item.size(); ++j)
            {
	...
	if (0 < cube_proto->gold)
		ch->ChangeGold(-(long long)cube_proto->gold);
#else
	if (0 < cube_proto->gold)
	...
	if ( percent_number<=cube_proto->percent)
	{
		...
		if (new_item->GetVnum() == 25422)
			ch->UpdateBattlePass(MISSION_FREE_ITEM_CRAFT, 1);
#endif
...
#endif
{
	...
	if ((inven_type == INVENTORY && INVENTORY_MAX_NUM<=inven_index) ||
		...
		(inven_type == POTIONS_INVENTORY && SPECIAL_INVENTORY_MAX_NUM<=inven_index) ||
	...
	if (inven_type != INVENTORY && inven_type != UPGRADE_INVENTORY && inven_type != POTIONS_INVENTORY && inven_type != CHEST_INVENTORY && inven_type != BONUS_INVENTORY)
		return;
#else
