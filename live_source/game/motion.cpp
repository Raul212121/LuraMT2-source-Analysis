...
// POLYMORPH_BUG_FIX
static float MSA_GetNormalAttackDuration(DWORD vnum, const char* msaPath)
{
	float duration = 99.0f;
	FILE * fp = fopen(msaPath, "rt");
	if (!fp)
	{
		sys_err("cannot open %s mob(%u)", msaPath, vnum);
		return duration;
	}
...
static float MOB_GetNormalAttackDuration(TMobTable* mobTable)
{
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		if (strcmp(mode, "GENERAL") == 0 && strncmp(type, "NORMAL_ATTACK", 13) == 0)
		{
			...
			float curDuration = MSA_GetNormalAttackDuration(mobTable->dwVnum, msaPath);
