...
int CShop::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	ch->ChangeGold(-static_cast<int64_t>(dwPrice));
	...
	if (m_pkPC)
	{
		...
		if (pkNewItem)
		{
			...
			else if (pkNewItem->IsPotions())
				pkNewItem->AddToCharacter(ch, TItemPos(POTIONS_INVENTORY, iEmptyPos));
			else if (pkNewItem->IsBonus())
				pkNewItem->AddToCharacter(ch, TItemPos(BONUS_INVENTORY, iEmptyPos));
			else if (pkNewItem->IsChest())
				pkNewItem->AddToCharacter(ch, TItemPos(CHEST_INVENTORY, iEmptyPos));
#endif
			else
