...
enum ESortSpecialInventoryType
{
	SORT_UPGRADE_INVENTORY,
	SORT_POTIONS_INVENTORY,
	SORT_BONUS_INVENTORY,
	SORT_CHEST_INVENTORY,
	SORT_INVENTORY_MAX_NUM,
};
#endif
...
typedef struct character_point_instant
{
	struct PlayerSlotT
	{
		...
		LPITEM pPotItems[SPECIAL_INVENTORY_MAX_NUM];
		WORD wPotItemGrid[SPECIAL_INVENTORY_MAX_NUM];

		LPITEM pBnsItems[SPECIAL_INVENTORY_MAX_NUM];
		WORD wBnsItemGrid[SPECIAL_INVENTORY_MAX_NUM];


		LPITEM pSINCtems[SPECIAL_INVENTORY_MAX_NUM];
		WORD wSINCtemGrid[SPECIAL_INVENTORY_MAX_NUM];
#endif
	...
	std::unique_ptr<PlayerSlotT> playerSlots;
...
class CHARACTER : public CEntity, public CFSM, public CHorseRider
{
	...
	public:
		...
		void 			ClearAffect(bool bSave = false, bool isDeadByMonster = false);
	...
	public:
		...
		void SetSortInventoryTime() { m_iSortInventoryTime = thecore_pulse(); }
		int GetSortInventoryTime() const { return m_iSortInventoryTime; }
#ifdef ENABLE_SPECIAL_INVENTORY
		void SortSpecialInventoryItems(BYTE type);
		void SetSortSpecialInventoryTime(BYTE type) { m_iSortSpecialInventoryTime[type] = thecore_pulse(); }
		int GetSortSpecialInventoryTime(BYTE type) const { return m_iSortSpecialInventoryTime[type]; }
#endif
#endif
		...
		LPITEM			GetPotionsInventoryItem(WORD wCell) const;
		...
		int				GetEmptyPotionsInventory(LPITEM pItem) const;
		int				GetEmptyBonusInventory(LPITEM pItem) const;
...
#ifdef ENABLE_SORT_INVENTORY
		int m_iSortInventoryTime;
#ifdef ENABLE_SPECIAL_INVENTORY
		int m_iSortSpecialInventoryTime[SORT_INVENTORY_MAX_NUM];
#endif
#endif
...
//		AttackLog			m_kAttackLog;
	...
	public:
		...
		bool			Return(bool regen_pos = false);
	...
	inline bool CompareIP(LPCHARACTER ch2);
...
protected:
	...
	private:
		int m_dwUseItemAntiFloodPulse;
		DWORD m_dwUseItemAntiFloodCount;
	public:
		int GetUseItemAntiFloodPulse() const { return m_dwUseItemAntiFloodPulse; }
		DWORD GetUseItemAntiFloodCount() const { return m_dwUseItemAntiFloodCount; }
		DWORD IncreaseUseItemAntiFloodCount() { return ++m_dwUseItemAntiFloodCount; }
		void SetUseItemAntiFloodPulse(int dwPulse) { m_dwUseItemAntiFloodPulse = dwPulse; }
		void SetUseItemAntiFloodCount(DWORD dwCount) { m_dwUseItemAntiFloodCount = dwCount; }
		bool CheckUseItemAntiFlood(LPITEM pItem);
...
public:
	void UpdateBattlePass(uint8_t mission, int64_t value);

	void RewardFinalBattlePass(uint8_t mission);
	void SkipBattleMission(uint8_t mission);

	void GetBattlePass();
	void DestroyBattlePass();

	void UnlockBattlePass(uint16_t id);
	void BattePassLogin();

	bool IsBattlePassFinished(uint8_t id);
	uint8_t GetBattleMission(uint8_t mission);

	private:
		std::map<uint8_t, TBattlePass> battlepass = {};
#endif
...
public:
	void SetDiscordUserId(const uint64_t userId) { m_DiscordUserId = userId; }
	[[nodiscard]] uint64_t GetDiscordUserId() const { return m_DiscordUserId; }
private:
	uint64_t m_DiscordUserId;


public:
	uint32_t GetShootSpeedPulse() const { return m_dwShootSpeedPulse; }
	uint32_t GetShootSpeedCount() const { return m_dwShootSpeedCount; }
	void SetShootSpeedPulse(uint32_t dwShootSpeedPulse) { m_dwShootSpeedPulse = dwShootSpeedPulse; }
	void SetShootSpeedCount(uint32_t dwShootSpeedCount) { m_dwShootSpeedCount = dwShootSpeedCount; }
	uint32_t IncreaseShootSpeedCount() { return ++m_dwShootSpeedCount; }
	bool IsShootSpeedHack(uint8_t seconds = 2, uint8_t count = 5);
private:
	uint32_t m_dwShootSpeedPulse;
	uint32_t m_dwShootSpeedCount;
};
