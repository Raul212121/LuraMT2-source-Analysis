...
DWORD AdjustExpByLevel(const LPCHARACTER ch, const DWORD exp)
{
	if (PLAYER_MAX_LEVEL_CONST < ch->GetLevel())
	{
		...
		for (int i=0 ; i < ch->GetLevel()-100 ; ++i)
		{
			if ( (i%10) == 0)
				factor /= 2.0;
...
void CHARACTER::Reward(bool bItemDrop) {
    ...
    if (pkAttacker->IsPC()) {
        if ((GetLevel() - pkAttacker->GetLevel()) >= -10) {
            if (pkAttacker->GetRealAlignment() < 0) {
                if (pkAttacker->IsEquipUniqueItem(
        ...
        quest::CQuestManager::instance().Kill(pkAttacker->GetPlayerID(),
                                              GetRaceNum());
        ...
        if (!number(0, 9)) {
            if (pkAttacker->GetPoint(POINT_KILL_HP_RECOVERY)) {
                int iHP = pkAttacker->GetMaxHP() *
                          pkAttacker->GetPoint(POINT_KILL_HP_RECOVERY) / 100;
                pkAttacker->PointChange(POINT_HP, iHP);
                CreateFly(FLY_HP_SMALL, pkAttacker);
            }

            if (pkAttacker->GetPoint(POINT_KILL_SP_RECOVER)) {
                int iSP = pkAttacker->GetMaxSP() *
                          pkAttacker->GetPoint(POINT_KILL_SP_RECOVER) / 100;
    ...
    if (!SECTREE_MANAGER::instance().GetMovablePosition(GetMapIndex(), pos.x,
                                                        pos.y, pos))
        return;
    ...
    if (item_rest_size) {
        if (s_vec_item.size() == 0)
            ...
            item->AddToGround(GetMapIndex(), pos);
            if (CBattleArena::instance().IsBattleArenaMap(
                    pkAttacker->GetMapIndex()) == false) {
            ...
            std::vector<std::pair<CHARACTER*, int64_t>> characterToDamageTable;

            int64_t totalDamage = 0;
            for (auto it = std::begin(m_map_kDamage); it != std::end(m_map_kDamage); ++it) {
                auto ch = CHARACTER_MANAGER::Instance().Find(it->first);
                if (!ch)
                    continue;

                characterToDamageTable.emplace_back(ch, it->second.iTotalDamage);
                totalDamage += it->second.iTotalDamage;
            }
            ...
            for (const auto& item : s_vec_item) {
                ...
                auto ch = GetRandomCharacterFromWeightedTable(characterToDamageTable, totalDamage);
                if (ch) {
                    if (ch->GetParty())
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
...
#ifdef ENABLE_WORLD_BOSS
		if (IsMonster())
			CWorldBossManager::instance().OnKillWorldBoss(this);
#endif
#ifdef ENABLE_BATTLE_PASS
		const std::map<DWORD, uint8_t> killmap = {
			{1901, MISSION_FREE_BOSS_SLAYER_1},
			{1093, MISSION_FREE_BOSS_SLAYER_2},
			{2493, MISSION_FREE_BOSS_SLAYER_3},
			{2598, MISSION_FREE_BOSS_SLAYER_4},
		};
		auto kill = killmap.find(GetRaceNum());
		if (kill != killmap.end())
			pkKiller->UpdateBattlePass(kill->second, 1);

		bool isEligibleForStones = std::abs(pkKiller->GetLevel() - GetLevel()) <= 5;
		if (IsStone() && isEligibleForStones)
			pkKiller->UpdateBattlePass(MISSION_FREE_STONE_BREAKER, 1);

#endif
#ifdef ENABLE_RANKING_SYSTEM
		if (IsPC())
			CRankingSystem::Instance().UpdateRankingdata(pkKiller, CATEGORY_PLAYERS, 1);
		else if (IsStone())
			CRankingSystem::Instance().UpdateRankingdata(pkKiller, CATEGORY_METINS, 1);
		else if (GetMobRank() >= MOB_RANK_BOSS)
			CRankingSystem::Instance().UpdateRankingdata(pkKiller, CATEGORY_BOSSES, 1);
		else if (IsMonster())
			CRankingSystem::Instance().UpdateRankingdata(pkKiller, CATEGORY_MONSTERS, 1);
#endif
...
#ifdef ENABLE_BATTLE_PASS
			if (pkKiller->IsPC() && std::abs(pkKiller->GetLevel() - GetLevel()) <= 15)
				pkKiller->UpdateBattlePass(MISSION_FREE_MASSACRE, 1);
#endif
			CGuild * g1 = GetGuild();
...
#ifdef ENABLE_PC_ITEM_DROP_PENALTY
	if (pkKiller &&
			!isAgreedPVP &&
			!isUnderGuildWar &&
			IsPC() &&
    ...
    ClearAffect(true, !(pkKiller && pkKiller->IsPC()));
	...
	{
		quest::PC* pPC = quest::CQuestManager::instance().GetPC(pkKiller->GetPlayerID());

		if (!pPC)
			return;

		int iValueBio = pPC->GetFlag("biolog.complete");
		if (iValueBio >= 0 && iValueBio < MISSION_MAX_BIOLOG)
		{
			CBiolog::instance().ObjectDrop(pkKiller, this, GetMobTable().dwVnum, iValueBio);
		}
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	//puRest2.Pop();
#ifdef ENABLE_BATTLE_PASS
	if (!IsPC() && pAttacker && pAttacker->IsPC() && !pAttacker->IsPolymorphed())
		pAttacker->UpdateBattlePass(MISSION_DAMAGE, dam);
#endif
...
void CHARACTER::FlyTarget(DWORD dwTargetVID, long x, long y, BYTE bHeader)
{
	...
	if (pkVictim)
	{
		if (IsPC())
		{
			if (!IsVictimInView(pkVictim) || !battle_is_attackable(this, pkVictim)) // by motz
				return;
		}
