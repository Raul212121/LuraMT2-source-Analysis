...
void CItem::SetAttribute(int i, BYTE bType, short sValue)
{
	...
	if (bType)
	{
		...
		LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(i, bType, sValue, GetID(), "SET_ATTR", "", pszIP ? pszIP : "", GetOriginalVnum(), GetOwner() ? GetOwner()->GetDiscordUserId() : 0));
	}
}
...
void CItem::SetForceAttribute(int i, BYTE bType, short sValue)
{
	...
	if (bType)
	{
		...
		LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(i, bType, sValue, GetID(), "SET_FORCE_ATTR", "", pszIP ? pszIP : "", GetOriginalVnum(), GetOwner() ? GetOwner()->GetDiscordUserId() : 0));
	}
}
...
bool CItem::ChangeRareAttribute()
{
	...
	if (GetOwner() && GetOwner()->GetDesc())
		...
		LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(0, 0, 0, GetID(), "SET_RARE_CHANGE", "", "", GetOriginalVnum(), GetOwner() ? GetOwner()->GetDiscordUserId() : 0))
...
bool CItem::AddRareAttribute()
{
	...
	LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(pos, attr.bType, attr.sValue, GetID(), "SET_RARE", "", pszIP ? pszIP : "", GetOriginalVnum(), GetOwner() ? GetOwner()->GetDiscordUserId() : 0));
	return true;
}
...
void CItem::PutRareAttributeWithLevel(BYTE bLevel)
{
	...
	if (avail.empty()) // by motz
	{
		sys_err("Cannot put item rare attribute %d %d %s item_id(%u)", iAttributeSet, bLevel, GetName(), GetID());
		return;
	}
