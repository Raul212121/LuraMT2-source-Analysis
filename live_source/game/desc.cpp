...
void DESC::Initialize()
{
	...
	m_bAdminMode = false;
	m_bPong = true;
	m_channelStatusRequestedTime = get_global_time();
...
void DESC::Destroy()
{
	...
	const auto& account = GetAccountTable();
	if (m_bLoginKeyExpired && account.id > 0)
	{
		TPacketExpiredLoginKey packet;
		strlcpy(packet.login, account.login, sizeof(packet.login));
		packet.dwLoginKey = GetLoginKey();
		db_clientdesc->DBPacket(HEADER_GD_EXPIRED_LOGIN_KEY, 0, &packet, sizeof(packet));
	}
...
EVENTFUNC(ping_event)
{
	...
	if (IsAdminPage(desc->GetHostName()))
	...
	else
	{
		...
		if (iSizeBuffer > 0)
		{
			...
			int iSizeAfter = TEA_Decrypt((uint32_t*) buffer_write_peek(lpBufferDecrypt),
					...
					(const uint32_t*)GetDecryptionKey(),
					iSizeBuffer);
		...
		else
		{
			if (buffer_has_space(m_lpOutputBuffer) < iSize + 8)
			{
				...
				if (buffer_has_space(m_lpOutputBuffer) < iSize + 8)
				{
					...
					sys_err("buffer mem_size overflow : memsize(%u) write_pos(%u) iSize(%d) m_iPhase(%u) header(%u) who(%s)", m_lpOutputBuffer->mem_size, m_lpOutputBuffer->write_point_pos, iSize, m_bPhase, header, name.c_str());
					m_bPhase = PHASE_CLOSE;
				}
			}
			else
			{
				...
				if (packet_encode(m_lpOutputBuffer, c_pvData, iSize))
				{
					int iSize2 = TEA_Encrypt((uint32_t*)pdwWritePoint, (uint32_t*)pdwWritePoint, (const uint32_t*)GetEncryptionKey(), iSize);
					if (iSize2 > iSize)
...
void DESC::SetPhase(BYTE phase)
{
	...
	switch (m_bPhase)
	{
		...
		case PHASE_GUILD_MARK:
#ifndef _IMPROVED_PACKET_ENCRYPTION_
			m_bEncrypted = true;
#endif
			m_pInputProcessor = &m_inputGuildMark;
			break;
...
bool DESC::IsChannelStatusRequested() const
{
	constexpr uint32_t delay = 30; // sec
	return get_global_time() - m_channelStatusRequestedTime > delay;
}
