...
static void __DropEvent_CharStone_DropItem(CHARACTER & killer, CHARACTER & victim, ITEM_MANAGER& itemMgr, std::vector<LPITEM>& vec_item)
{
	...
	if (number(1, MaxRange) <= dropPercent)
	{
		...
		int item_vnum = Stones[number(0, _countof(Stones) - 1)];
...
bool ITEM_MANAGER::GetTargetInfoDrop(LPCHARACTER ch, LPCHARACTER victim, std::vector<SItemInfoDrop>& vecItemInfoDrop)
{
	if (!ch || !victim)
		return false;

	const auto level = ch->GetLevel();
	const auto race_id = victim->GetRaceNum();

	const auto rank = victim->GetMobRank();
	if (rank < MOB_RANK_MAX_NUM)
	{
		for (const auto& v : g_vec_pkCommonDropItem[rank])
		{
			if (level < v.m_iLevelStart || level > v.m_iLevelEnd)
				continue;

			TItemTable* table = GetTable(v.m_dwVnum);
			if (!table)
				continue;

			if (v.m_dwVnum > 70103 && v.m_dwVnum < 70108)
				if (v.m_dwVnum != victim->GetPolymorphItemVnum())
					continue;

			SItemInfoDrop info = {};
			info.vnum = v.m_dwVnum;
			info.count++;
			info.level_min = v.m_iLevelStart;
			info.level_max = v.m_iLevelEnd;
			info.percent = v.m_iPercent;
			vecItemInfoDrop.emplace_back(info);
		}
	}

	if (m_map_pkDropItemGroup.find(race_id) != m_map_pkDropItemGroup.end())
	{
		const auto group = m_map_pkDropItemGroup[race_id];
		if (group != nullptr)
		{
			for (const auto& v : group->GetVector())
			{
				SItemInfoDrop info = {};
				info.vnum = v.dwVnum;
				info.count = v.iCount;
				info.percent = v.dwPct;
				vecItemInfoDrop.emplace_back(info);
			}
		}
	}

	if (m_map_pkMobItemGroup.find(race_id) != m_map_pkMobItemGroup.end())
	{
		const auto group = m_map_pkMobItemGroup[race_id];
		if (group != nullptr && !group->IsEmpty())
		{
			for (const auto& v : group->GetVector())
			{
				SItemInfoDrop info = {};
				info.vnum = v.dwItemVnum;
				info.count = v.iCount;
				info.percent = v.iRarePct;
				vecItemInfoDrop.emplace_back(info);
			}
		}
	}

	if (m_map_pkLevelItemGroup.find(race_id) != m_map_pkLevelItemGroup.end())
	{
		const auto group = m_map_pkLevelItemGroup[race_id];
		if (group != nullptr)
		{
			if (static_cast<int>(group->GetLevelLimit()) <= level)
			{
				for (const auto& v : group->GetVector())
				{
					SItemInfoDrop info = {};
	...
	if (ch->GetPremiumRemainSeconds(PREMIUM_ITEM) > 0 || ch->IsEquipUniqueGroup(UNIQUE_GROUP_DOUBLE_ITEM))
	{
		const auto iterator = m_map_pkGloveItemGroup.find(race_id);
		if (iterator != m_map_pkGloveItemGroup.end())
		{
			for (auto& v : iterator->second->GetVector())
			{
				SItemInfoDrop info = {};
				info.vnum = v.dwVnum;
				info.count = v.iCount;
	...
	if (victim->GetMobDropItemVnum() > 0)
	{
		const auto iterator = m_map_dwEtcItemDropProb.find(victim->GetMobDropItemVnum());
		if (iterator != m_map_dwEtcItemDropProb.end())
		{
			SItemInfoDrop info = {};
			info.vnum = victim->GetMobDropItemVnum();
			info.count++;
			vecItemInfoDrop.emplace_back(info);
		}
	}
	...
	if (victim->IsStone())
	{
		if (victim->GetDropMetinStoneVnum())
		{
			SItemInfoDrop info = {};
			info.vnum = victim->GetDropMetinStoneVnum();
			info.count++;
			vecItemInfoDrop.emplace_back(info);
		}
	}

	return !vecItemInfoDrop.empty();
}
#endif
