...
#	define INTRUSIVE_HANDSHAKE_LOG TRUE /* (choice)
...
#	define INTRUSIVE_HANDSHAKE_PULSE 1 /* (seconds)
	...
	* what you're doing.
	*/
#	define INTRUSIVE_HANDSHAKE_NEXT_PULSE 60 * 5 /* (seconds)
	...
	*/
#	define INTRUSIVE_HANDSHAKE_LIMIT 2 /* (handshakes)
...
class DESC
{
	...
	public:
		...
		virtual void		SetPhase(BYTE phase);
		...
		bool			IsPhase(const BYTE phase) const { return m_bPhase == phase; }
		...
		BYTE			GetEmpire();
		[[nodiscard]] uint64_t GetDiscordUserId() const { return m_accountTable.discordUserId; }
		...
		void			SetChannelStatusRequested(uint32_t channelStatusRequestedTime) { m_channelStatusRequestedTime = channelStatusRequestedTime; }
		void			SetNewEmpire(int iEmpire) { m_iNewEmpire = iEmpire; }
		int				GetNewEmpire() { return m_iNewEmpire; }
	...
	protected:
		...
		CInputGuildMark m_inputGuildMark;
		...
		bool			m_bDestroyed;
		uint32_t		m_channelStatusRequestedTime;
