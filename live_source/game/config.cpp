...
std::set<std::string> g_stAdminPageIP;
std::string	g_stAdminPagePassword = "KvvpgBDI4iaEjPI4";
...
{
	return g_stAdminPageIP.find(ip) != g_stAdminPageIP.end();
}
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fpOnlyForDB))
	{
		...
		TOKEN("auth_server")
		{
			...
			if (!*szIP || (!*szPort && strcasecmp(szIP, "master") != 0))
			{
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("guild_max_members")
		{
			DWORD flag = 0;
			str_to_number(flag, value_string);

			g_dwGuildMaxMembers = flag;
			fprintf(stdout, "GUILD_MAX_MEMBERS: %u\n", g_dwGuildMaxMembers);
			continue;
		}
