...
void CHARACTER_MANAGER::Update(int iPulse)
{
	...
	if (GetCharactersByRaceNum(xmas::MOB_SANTA_VNUM, i))
		std::for_each(i.begin(), i.end(), [iPulse](LPCHARACTER ch) { if (ch != nullptr) { ch->UpdateStateMachine(iPulse); } });
...
void CHARACTER_MANAGER::CheckEventForDrop(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM>& vec_item)
{
	...
	eventPtr = CheckEventIsActive(HEXAGONAL_EVENT, killerEmpire);
	if (eventPtr)
	{
		const int prob = number(1, 100);
		const int success_prob = eventPtr->value[3];
		if (prob < success_prob)
		{
			LPITEM item = ITEM_MANAGER::Instance().CreateItem(50124, 1, 0, true);
...
void CHARACTER_MANAGER::SetEventStatus(const uint16_t eventID, const bool eventStatus, const int endTime, const char* endTimeText)
{
	...
	const std::map<uint8_t, std::pair<std::string, std::string>> m_eventText = {
		{BONUS_EVENT,{"Eveniment-ul cu bonusuri a fost activat","Eveniment-ul cu bonusuri a fost dezactivat"}},
		{DOUBLE_BOSS_LOOT_EVENT,{"Sansa de drop dublu la Bossi a fost activat!","Sansa de drop dublu la Bossi a fost dezactivat!"}},
		{DOUBLE_METIN_LOOT_EVENT,{"Sansa de drop dublu la Pietrele Metin a fost activata!","Sansa dubla la dropul Pietrelor Metin a fost dezactivata"}},
		{HEXAGONAL_EVENT,{"Eveniment-ul cu cufere hexagonale a fost activat","Eveniment-ul cu cufere hexagonale a fost dezavtivat"}},
		{MOONLIGHT_EVENT,{"Eveniment-ul cu cufere clar de luna a fost activat","Eveniment-ul cu cufere clar de luna a fost dezactivat"}},

		{EVENT_FISHING,{"Eveniment-ul de pescuit a fost activat","Eveniment-ul de pescuit a fost dezactivat"}},
		{EVENT_PVM,{"Eveniment-ul PvM a fost activat, apasa pe casuta din fereastra de evenimente pentru a te teleporta","Eveniment-ul PvM fost dezactivat"}},
	};
	...
	if (eventData->eventIndex == EVENT_FISHING)
		quest::CQuestManager::instance().RequestSetEventFlag("eveniment_pescuit", eventStatus);
