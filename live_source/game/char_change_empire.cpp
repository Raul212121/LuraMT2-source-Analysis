...
void CHARACTER::SetChangeEmpireCount()
{
	...
	{
		count++;
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO change_empire (account_id, change_count, time) VALUES(%u, %d, NOW())", dwAID, count);
	}
	else
	{
		count++;
		snprintf(szQuery, sizeof(szQuery), "UPDATE change_empire SET change_count=%d, time=NOW() WHERE account_id=%u", count, dwAID);
	}
