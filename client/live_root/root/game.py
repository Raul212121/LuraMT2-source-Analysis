...
class GameWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		self.litere_event.SetPosition(wndMgr.GetScreenWidth()-290,20)
		self.litere_event.SetUpVisual("d:/ymir work/eventlitere/litere_inchis.tga")
		self.litere_event.SetOverVisual("d:/ymir work/eventlitere/litere_deschis.tga")
		self.litere_event.SetDownVisual("d:/ymir work/eventlitere/litere_deschis.tga")
		self.litere_event.SetEvent(lambda : net.SendChatPacket("/letters_open"))

		if app.ENABLE_EASTER_EVENT:
			self.easterEvent = ui.Button()
			self.easterEvent.SetParent(self)
			self.easterEvent.SetPosition(wndMgr.GetScreenWidth()-225, 15)
			self.easterEvent.SetUpVisual("d:/ymir work/ui/game/easter_event/button_open1.png")
			self.easterEvent.SetOverVisual("d:/ymir work/ui/game/easter_event/button_open2.png")
			self.easterEvent.SetDownVisual("d:/ymir work/ui/game/easter_event/button_open2.png")
			self.easterEvent.SetEvent(lambda : self.interface.OpenEasterEventBoard())
	...
	def Close(self):
		...
		self.buttonInvSearchShop = None
		self.buttonInvSwitchBot = None
		self.buttonSpecialStorage = None
		self.buttonMiniMap = None
		self.buttonTeleport = None
		self.buttonTemnite = None
	...
	def __BuildKeyDict(self):
		...
		onPressKeyDict[app.DIK_F8]= lambda : self.interface.RankingWindowOpen()
		onPressKeyDict[app.DIK_F9] = lambda : net.SendChatPacket("/letters_open")
		onPressKeyDict[app.DIK_U]	= lambda : self.interface.ToggleSpecialStorageWindow()
		onPressKeyDict[app.DIK_F5]	= lambda : self.interface.ToggleHourEvent()
		onPressKeyDict[app.DIK_F6]			= lambda : self.interface.ToggleWikiNew()
		if app.ENABLE_HUNTING_SYSTEM:
			onPressKeyDict[app.DIK_F7]			= lambda : self.interface.ToggleHuntingWindow()
	...
	if app.ENABLE_MOVE_CHANNEL:
		...
		def __SeverInfo(self, channelNumber, mapIndex):
			...
			net.SetServerInfo(("%s, %s " % (_svName, "CH99" if _chNum == 99 else _chName[_chNum]["name"])).strip())
		...
		if mouseModule.mouseController.isAttached():
			...
			if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
				if player.SLOT_TYPE_UPGRADE_INVENTORY == attachedType or\
					player.SLOT_TYPE_POTIONS_INVENTORY == attachedType or\
		...
		if player.SLOT_TYPE_INVENTORY == attachedType or\
			...
			if True == chr.HasInstance(self.PickingCharacterIndex) and player.GetMainCharacterIndex() != dstChrID:
				if player.IsEquipmentSlot(attachedItemSlotPos) and\
					...
					player.SLOT_TYPE_POTIONS_INVENTORY != attachedType and\
					player.SLOT_TYPE_BONUS_INVENTORY != attachedType and\
					player.SLOT_TYPE_CHEST_INVENTORY != attachedType:
					self.stream.popupWindow.Close()
		...
		if player.SLOT_TYPE_INVENTORY == attachedType and player.IsEquipmentSlot(attachedItemSlotPos):
			...
			if player.SLOT_TYPE_INVENTORY == attachedType or \
				...
				if app.ENABLE_NEW_DROP_DIALOG:
					...
					itemDropQuestionDialog.SetCancelEvent(lambda arg=False: self.RequestDropItem(arg))
					itemDropQuestionDialog.SetItemSlot(player.SlotTypeToInvenType(attachedType),attachedItemSlotPos)
					itemDropQuestionDialog.Open()
					itemDropQuestionDialog.dropType = attachedType
		...
		if answer:
			...
			if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
				if player.SLOT_TYPE_UPGRADE_INVENTORY == dropType or\
					player.SLOT_TYPE_POTIONS_INVENTORY == dropType or\
					player.SLOT_TYPE_BONUS_INVENTORY == dropType or\
		...
		def RequestDestroyItem(self, answer):
			...
			if answer:
				...
				if player.SLOT_TYPE_INVENTORY == dropType:
					if dropNumber == player.ITEM_MONEY:
						return
					elif dropNumber == player.ITEM_MONEY:
						return
					else:
				...
				if app.ENABLE_SPECIAL_INVENTORY_SYSTEM:
					if player.SLOT_TYPE_UPGRADE_INVENTORY == dropType or\
						...
						player.SLOT_TYPE_BONUS_INVENTORY == dropType or\
						player.SLOT_TYPE_CHEST_INVENTORY == dropType:
						self.__SendDestroyItemPacket(dropNumber, player.SlotTypeToInvenType(dropType))
	...
	def OnUpdate(self):
		...
		if app.ENABLE_EASTER_EVENT:
			if constInfo.EASTER_EVENT_STATUS == 1:
				self.easterEvent.Show()
			else:
				self.easterEvent.Hide()
	...
	if app.ENABLE_TARGET_INFO_DROP:
	...
	def __ServerCommand_Build(self):
		serverCommandList={
			...
			"ShowSpecialInventory"	: self.OpenSpecialInventory,
			...
			"teleportRing"		:self.TeleportareQuest,
		...
		if app.ENABLE_MOVE_CHANNEL:
			serverCommandList.update({"server_info" : self.__SeverInfo})
		...
		if app.ENABLE_EASTER_EVENT:
			self.serverCommander.SAFE_RegisterCallBack("set_status_easter_event", self.SetStatusEasterEvent)
	...
	if app.ENABLE_BIOLOG_SYSTEM:
		def BINARY_BIOLOG_DONE(self):
			self.interface.BiologDoneBonus()

		def BINARY_BIOLOG_TIME(self, time_left):
			self.interface.AppendInfoBiologTime(time_left)

		def BINARY_BIOLOG(self, iVnumReward, iReqVnum, iNeedCount, iReqRemain, iReqLevel):
			self.interface.AppendInfoBiolog(iVnumReward, iReqVnum, iNeedCount, iReqRemain, iReqLevel)

		def BINARY_BIOLOG_REWARD_BONUS(self, value):
			self.interface.AppendBonusBiolog(value)
	...
	def TeleportareQuest(self, questindex):
		constInfo.QUESTNAMEINDEX = questindex

	def __TeleportOpen(self):
		import event
	...
	if app.ENABLE_EASTER_EVENT:
		def SetStatusEasterEvent(self, iStatus):
			constInfo.EASTER_EVENT_STATUS = int(iStatus)

	def OpenSpecialInventory(self):
		self.interface.ToggleSpecialStorageWindow()

	def BINARY_RefreshBattlePass(self):
		if self.interface:
			self.interface.wndBattlePassNew.UpdateBattlePass()
	...
	def BINARY_AddPlayerRanking(self, rank, name, guild, empire, count, selfpos):
		if self.interface:
			self.interface.rankingWindow.AddPlayerRanking(rank, name, guild, empire, count, selfpos)
	
	def BINARY_ClearPlayerRanking(self):
		if self.interface:
			self.interface.rankingWindow.ClearPlayerRanking()
