...
class EnergyBar(ui.ScriptWindow):
	class TextToolTip(ui.Window):
		def __init__(self, CursorDistanceY = 15):
			ui.Window.__init__(self, "TOP_MOST")
			...
			textLine.Show()
			self.textLine = textLine
			self.CursorDistanceY = CursorDistanceY
		...
		def OnRender(self):
			(mouseX, mouseY) = wndMgr.GetMousePosition()
			self.textLine.SetPosition(mouseX, mouseY - self.CursorDistanceY)

	def __init__(self):
		#print "NEW TASKBAR  ----------------------------------------------------------------------------"
		ui.ScriptWindow.__init__(self)
		self.tooltipEnergy = self.TextToolTip(30)
		self.tooltipEnergy.Show()

		self.tooltipTimeLeft = self.TextToolTip()
		self.tooltipTimeLeft.Show()
	...
	def LoadWindow(self):
		...
		self.energy5 = self.GetChild("EnergyGauge_5")
		self.energy10 = self.GetChild("EnergyGauge_10")
		self.energy20 = self.GetChild("EnergyGauge_20")
		self.energy40 = self.GetChild("EnergyGauge_40")
		self.energy50 = self.GetChild("EnergyGauge_50")
		self.energy60 = self.GetChild("EnergyGauge_60")
		self.energy80 = self.GetChild("EnergyGauge_80")

		self.energyGaugeBoard = self.GetChild("EnergyGauge_Board")
		self.energyGaugeToolTip = self.GetChild("EnergyGauge_ToolTip")

	def HideAllGauges(self):
	...
	def RefreshStatus(self):
		...
		self.SetEnergy (pointEnergy, leftTimeEnergy, 1800) #maxtime set to 1800
	...
	def SetEnergy (self, point, leftTime, maxTime):
		...
		#	self.energyFull.Hide()
		elif ((leftTime * 100) / maxTime) < 5:
			self.HideAllGauges()
			self.energy5.Show()
		elif ((leftTime * 100) / maxTime) < 10:
			self.HideAllGauges()
			self.energy10.Show()
		elif ((leftTime * 100) / maxTime) < 20:
			self.HideAllGauges()
			self.energy20.Show()
		elif ((leftTime * 100) / maxTime) < 40:
			self.HideAllGauges()
			self.energy40.Show()
		elif ((leftTime * 100) / maxTime) < 50:
			self.HideAllGauges()
			self.energy50.Show()
		elif ((leftTime * 100) / maxTime) < 60:
			self.HideAllGauges()
			self.energy60.Show()
		elif ((leftTime * 100) / maxTime) < 80:
			self.HideAllGauges()
			self.energy80.Show()
		...
		#self.tooltipEnergy.SetText("%s. %s : %s" % (localeInfo.TOOLTIP_ENERGY % (point), localeInfo.LEFT_TIME, localeInfo.SecondToDHM(leftTime)))
		self.tooltipEnergy.SetText("%s" % (localeInfo.TOOLTIP_ENERGY_TASKBAR % (point)))
		self.tooltipTimeLeft.SetText("%s : %s" % (localeInfo.LEFT_TIME, localeInfo.SecondToDHM(leftTime)))
	...
	def __OnToolTipMouseOverOut(self):
		print("OnMouseOverOut")
		self.tooltipEnergy.Hide()
		self.tooltipTimeLeft.Hide()
