...
class Window(object):
	...
	def __init__(self, layer = "UI"):	
		...
		if app.ENABLE_MOUSEWHEEL_INPUT:
...
class ImageBoxRender(Window):
	...
	def SetEvent(self, func, *args):
		return
class ExpandedImageBoxRender(ImageBoxRender):
	def __init__(self, layer = "UI"):
		ImageBoxRender.__init__(self, layer)

	def __del__(self):
		ImageBoxRender.__del__(self)
...
class AniImageBox(Window):
	...
	def SetNextFrameEvent(self, event, *args):
		self.nextFrameEvent = event
...
class NewDragonBoard(Window):
	...
	def __init__(self):
		Window.__init__(self)
		self.MakeBoard("d:/ymir work/ui/interface/newdragonetc/Board_Corner_", "d:/ymir work/ui/interface/newdragonetc/Board_Line_")
		self.MakeBase()
	...
	def MakeBase(self):
		self.Base = ExpandedImageBox()
		self.Base.AddFlag("not_pick")
		self.Base.LoadImage("d:/ymir work/ui/interface/newdragonetc/Board_Base.tga")
		self.Base.SetParent(self)
		self.Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		self.Base.Show()
...
class NewDragonBoardWithTitleBar(NewDragonBoard):
	def __init__(self):
		NewDragonBoard.__init__(self)
	...
	def SetSize(self, width, height):
		...
		NewDragonBoard.SetSize(self, width, height)
...
class ScrollBarNew(Window):
	...
	class MiddleBar(DragButton):
		...
		def MakeImage(self):
			middle = ExpandedImageBox()
			middle.SetParent(self)
			middle.LoadImage("d:/ymir work/ui/cw/ranking/0.tga")
		...
		def SetImageForce(self, image):
			self.middle.LoadImage(image)
	...
	def GetPageScale(self):
		return self.SCROLLBAR_MIDDLE_HEIGHT / float(self.GetHeight())
	
	def SetBackgroundImage(self, image):
		self.barSlot.LoadImage(image)
	...
	def CreateScrollBar(self):
		barSlot = ImageBox()
		barSlot.SetParent(self)
		barSlot.LoadImage("d:/ymir work/ui/cw/ranking/1.tga")
		barSlot.AddFlag("not_pick")
		barSlot.Show()
	...
	def SetMiddleBarSize(self, pageScale):
		...
		self.mbHeight = realHeight

	def SetScrollBarSize(self, height):
		self.pageSize = (height) - self.SCROLLBAR_MIDDLE_HEIGHT
		self.SetSize(self.SCROLLBAR_WIDTH, height)
		self.middleBar.SetRestrictMovementArea(self.MIDDLE_BAR_POS, self.MIDDLE_BAR_POS-2, self.MIDDLE_BAR_POS, height)
		self.middleBar.SetPosition(self.MIDDLE_BAR_POS, 0)
	...
	def UpdateBarSlot(self):
		if not self.NEW_UPDATE:
			self.barSlot.SetPosition(1, 2)
	...
	def OnMouseWheel(self, len):
		length = float(len) / 120.0 * float(self.scrollStep)
		self.SetPos(self.curPos - length)
		return True

	def SetCustomBar(self, image):
		self.middleBar.SetImageForce(image)
...
class PythonScriptLoader(object):
	...
	def LoadScriptFile(self, window, FileName):
		...
		try:
			if __USE_CYTHON__ and IsInUiPath(FileName) and rootlib.isExist(modname):
				m1 = rootlib.moduleImport(modname)
	...
	def LoadChildren(self, parent, dicChildren):
		...
		for ElementValue in ChildrenList:
			...
			elif Type == "new_dragonboard":
				parent.Children[Index] = NewDragonBoard()
				parent.Children[Index].SetParent(parent)
				self.LoadElementBoard(parent.Children[Index], ElementValue, parent)

			elif Type == "new_dragonboard_with_titlebar":
				parent.Children[Index] = NewDragonBoardWithTitleBar()
				parent.Children[Index].SetParent(parent)
				self.LoadElementBoardWithTitleBar(parent.Children[Index], ElementValue, parent)
	...
	def LoadElementScrollBarNew(self, window, value, parentWindow):
		...
		if True == value.has_key("bar_image"):
			window.SetCustomBar(value["bar_image"])
...
def MakeImageTextLine(parent,image,image_x,image_y,text,text_x=0,text_y=0,center=1,color=None):
	img=MakeImageBox(parent,image,image_x,image_y)
	txt=MakeTextLine(img, center, center,text_x,text_y)
	txt.SetText(text)
	if color:
