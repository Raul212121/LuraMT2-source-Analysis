...
class MessengerGroupItem(MessengerItem):
	...
	def GetLoginMemberList(self):
		...
		return filter(lambda arg: arg.IsOnline(), self.memberList)
...
class MessengerWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.addBlockButton = self.GetChild("BlockButton")
		except:
			import exception
	...
	def ClearGuildMember(self):
		self.groupList[messenger.MESSENGER_GROUP_INDEX_GUILD].ClearMember()
	...
	def OnAddFriend(self):
		...
		if text:
			net.SendMessengerAddByNamePacket(text, False)
	...
	def OnLogin(self, groupIndex, key, name=None):
		...
		if groupIndex in (messenger.MESSENGER_GROUP_INDEX_FRIEND, messenger.MESSENGER_GROUP_INDEX_GUILD):
