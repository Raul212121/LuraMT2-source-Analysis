...
class QuestDialog(ui.ScriptWindow):
	...
	def __init__(self,skin,idx):
		...
		self.AddFlag("movable") ## update 15.03.2019 - movable questboard :)
	...
	def AppendQuestion(self, name, idx):
		...
		posStartClose = name.find("#T#")
		posEndClose = name.find("#t#")
		if posStartClose != -1 and posEndClose != -1:
			closeName = str(name[posStartClose + 3:posEndClose])
			oldCloseName = "#T#" + str(closeName) + "#t#"
			name = name.replace(oldCloseName, localeInfo.GetVariableValue(closeName))
