...
class ChatLine(ui.EditLine):
	...
	def __SendChatPacket(self, text, type):
		links={}
		(isLink, acceptedLink) = self.GetLinks(text, links)
		if net.IsChatInsultIn(text):
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.CHAT_INSULT_STRING)
		elif isLink and not acceptedLink:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.CANT_SEND_LINK)
		else:
			if isLink and acceptedLink:
	...
	def GetLinks(self, string, ret):
		...
		valid_position = [7, 8, 4, 11, 12]
		found = False

		try:
			for websites in constInfo.AUTHORIZED_LINKS:
				reSearch = re.search(websites, str(ret.items()[0][0]))
				if reSearch and (reSearch.span()[0] in valid_position):
					found = True
					break
		except: pass

		return (bool(len(links) > 0), bool(found))
