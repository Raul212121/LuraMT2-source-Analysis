...
namespace offlineshop
{
	...
	std::string CreateShopCacheInsertShopQuery(DWORD dwOwnerID, DWORD dwDuration, const char* name, const TShopPosition& pos);
#else
	...
	bool CShopCache::RemoveItem(DWORD dwOwnerID, DWORD dwItemID)
	{
		...
		auto it = pCache->itemsmap.find(dwItemID);
	...
	bool CShopCache::LockSellItem(DWORD dwOwnerID, DWORD dwItemID, const TValutesInfo& valutes)
	{
		TShopCacheInfo* pCache;
		if (!Get(dwOwnerID, &pCache))
			return false;
		...
		if (it->second.price.illYang != valutes.illYang)
			return false;

#if defined(ENABLE_CHEQUE_SYSTEM) || defined(__ENABLE_CHEQUE_SYSTEM__)
		if (it->second.price.iCheque != valutes.iCheque)
			return false;
#endif
...
#ifdef __NEW_OFFLINESHOP_SPAWN__
	bool CShopCache::CreateShop(DWORD dwOwnerID, DWORD dwDuration, const char* szName, std::vector<TShopCacheItemInfo>& items, const TShopPosition& pos)
#else
	...
	{
		auto it = m_shopsMap.find(qi->dwOwnerID);
		if (it == m_shopsMap.end())
			return false;
...
#ifdef __NEW_OFFLINESHOP_SPAWN__
	bool CShopCache::PutShop(DWORD dwOwnerID, DWORD dwDuration, const char* szName, const TShopPosition& pos)
#else
	...
	void CShopCache::ShopDurationProcess()
	{
			...
			for (; it_item != end_item; it_item++) {
				...
				if (item_info.expiration == offlineshop::ExpirationType::EXPIRE_REAL_TIME) {
					if (now > item_info.alSockets[0])
						item_vec.emplace_back(it->first, it_item->first);
				}
		...
		for(auto i : vec)
			CClientManager::instance().OfflineshopExpiredShop(i);
	...
	bool CSafeboxCache::PutItem(DWORD dwOwnerID, DWORD dwItem, const TItemInfoEx& item)
	{
		...
		auto it = pSafebox->itemsmap.find(dwItem);
	...
	bool CSafeboxCache::RemoveItem(DWORD dwOwnerID, DWORD dwItemID)
	{
		...
		auto it = pSafebox->itemsmap.find(dwItemID);
		if (it == pSafebox->itemsmap.end())
			return false;
		...
		auto* qi = new SQueryInfoSafeboxRemoveItem;
		qi->dwOwnerID = dwOwnerID;
		qi->dwItemID = dwItemID;

		CDBManager::instance().ReturnQuery(query.c_str(), QID_OFFLINESHOP_SAFEBOX_DELETE_ITEM, 0, qi);
		return true;
	}
	...
	CSafeboxCache::TSafeboxCacheInfo* CSafeboxCache::LoadSafebox(DWORD dwPID)
	{
...

		{
			...
			while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				for(long & alSocket : item.alSockets)
					str_to_number(alSocket , row[col++]);

				for (auto & i : item.aAttr)
				{
		...
		auto it = m_safeboxMap.insert(std::make_pair(dwPID, safebox)).first;
		return &it->second;
	}
	...
	{
		auto it = m_mapOffer.find(dwOfferID);
		if (it == m_mapOffer.end())
			return false;
	...
	bool COfferCache::CancelOffer(DWORD dwOfferID, DWORD dwOwnerID)
	{
		...
		for(auto itID = vec.begin() ; itID!= vec.end(); itID++)
		{
			if (*itID == dwOfferID)
			{
				vec.erase(itID);
	...
	bool COfferCache::RemoveOffersByShopOwner(DWORD dwShopOwnerID)
	{
		...
		for (auto& itID : vec)
		{
			auto itCache = m_mapOffer.find(itID);
	...
	bool CAuctionCache::AuctionDurationProcess()
	{
		...
		for (auto owner : owners)
			CClientManager::instance().OfflineshopExpiredAuction(owner);
	...
	std::string CreateShopCacheInsertItemQuery(DWORD dwOwner, const CShopCache::TShopCacheItemInfo& rItem)
	{
		...
		for (auto alSocket : rItem.item.alSockets)
			len += snprintf(szQuery + len, sizeof(szQuery) - len,
				",%ld ", alSocket);

		for (auto i : rItem.item.aAttr)
			len += snprintf(szQuery + len, sizeof(szQuery) - len,
				", %d , %d ", i.bType, i.sValue);
	...
	{
		static char szQuery[256];
		char szEscapeString[OFFLINE_SHOP_NAME_MAX_LEN * 2 + 1];

		CDBManager::instance().EscapeString(szEscapeString, name, strlen(name));

#ifdef __NEW_OFFLINESHOP_SPAWN__
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO `player`.`offlineshop_shops` (`owner_id`, `duration`, `name`, `pos_x`, `pos_y`, `map_index`, `channel`) VALUES(%u, %u, '%s', %ld, %ld, %ld, %u);",
	...
	{
		static char szQuery[256];
		static char szEscapeString[OFFLINE_SHOP_NAME_MAX_LEN * 2 + 1];
		CDBManager::instance().EscapeString(szEscapeString, name, strlen(name));
	...
	std::string CreateSafeboxCacheInsertItemQuery(DWORD dwOwner, const TItemInfoEx& item)
	{
		...
		for (auto alSocket : item.alSockets)
			len += snprintf(szQuery + len, sizeof(szQuery) - len,
				",%ld ", alSocket);
	...
	std::string CreateOfferCacheInsertOfferQuery(const COfferCache::TOfferCacheInfo& rOffer)
	{
		static char szEscapeString[CHARACTER_NAME_MAX_LEN * 2 + 1];
	...
	std::string CreateAuctionCacheAddAuctionQuery(const TAuctionInfo& auction)
	{
		...
		static char szEscapeName[CHARACTER_NAME_MAX_LEN * 2 + 1];
		...
		for (auto alSocket : auction.item.alSockets)
			len += snprintf(szQuery + len, sizeof(szQuery) - len,
				",%ld ", alSocket);

		for (auto i : auction.item.aAttr)
			len += snprintf(szQuery + len, sizeof(szQuery) - len,
				",%u , %d ", i.bType, i.sValue);
