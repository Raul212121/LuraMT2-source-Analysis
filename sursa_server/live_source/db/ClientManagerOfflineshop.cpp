...
#endif
				{
					sys_err("cannot execute putsShop -> double shop id?! owner_id(%u)", dwOwner);
				}
			}
		}
	}

	{
		...
		if (pMsg->Get())
		{
			while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				for (long & alSocket : item.item.alSockets)
					str_to_number(alSocket, row[col++]);

				for (auto & i : item.item.aAttr)
				{
					str_to_number(i.bType,	row[col++]);
					str_to_number(i.sValue, row[col++]);
				}
				...
				if(!m_offlineshopShopCache.PutItem(dwOwnerID, dwItemID, item, isSold))
					sys_err("cannot putitem owner_id(%u), item_id(%u), didn't deleted items when closed?", dwOwnerID, dwItemID);
			}
		}
	}

	{
		...
		if (pMsg->Get())
		{
			while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				if (!m_offlineshopOfferCache.Puts(dwOfferID, offer))
					sys_err("cannot putofffer owner_id(%u), offer_id(%u) offer_item_id(%u)", offer.dwOwnerID, dwOfferID, offer.dwItemID);
			}
		}
	}
...
void CClientManager::RecvOfflineShopPacket(CPeer* peer,const char* data)
{
	...
	{
	case offlineshop::SUBHEADER_GD_BUY_ITEM:
		bRet = RecvOfflineShopBuyItemPacket(peer, data);
		break;
	case offlineshop::SUBHEADER_GD_BUY_LOCK_ITEM:
		bRet = RecvOfflineShopLockBuyItem(peer, data);
		break;
	case offlineshop::SUBHEADER_GD_EDIT_ITEM:
		bRet = RecvOfflineShopEditItemPacket(peer, data);
		break;
	case offlineshop::SUBHEADER_GD_REMOVE_ITEM:
		bRet = RecvOfflineShopRemoveItemPacket(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_ADD_ITEM:
		bRet = RecvOfflineShopAddItemPacket(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_SHOP_FORCE_CLOSE:
		bRet = RecvOfflineShopForceClose(peer, data);
		break;
	case offlineshop::SUBHEADER_GD_SHOP_CREATE_NEW:
		bRet = RecvOfflineShopCreateNew(peer, data);
		break;
	case offlineshop::SUBHEADER_GD_SHOP_CHANGE_NAME:
		bRet = RecvOfflineShopChangeName(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_OFFER_CREATE:
		bRet = RecvOfflineShopOfferCreate(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_OFFER_NOTIFIED:
		bRet = RecvOfflineShopOfferNotified(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_OFFER_ACCEPT:
		bRet = RecvOfflineShopOfferAccepted(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_OFFER_CANCEL:
		bRet = RecvOfflineshopOfferCancel(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_SAFEBOX_GET_ITEM:
		bRet = RecvOfflineShopSafeboxGetItem(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_SAFEBOX_GET_VALUTES:
		bRet = RecvOfflineShopSafeboxGetValutes(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_SAFEBOX_ADD_ITEM:
		bRet = RecvOfflineShopSafeboxAddItem(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_AUCTION_CREATE:
		bRet = RecvOfflineShopAuctionCreate(peer, data);
		break;

	case offlineshop::SUBHEADER_GD_AUCTION_ADD_OFFER:
		bRet = RecvOfflineShopAuctionAddOffer(peer, data);
		break;
...
bool CClientManager::RecvOfflineShopLockBuyItem(CPeer* peer, const char* data)
{
	...
	offlineshop::TValutesInfo valutes = subpack->valutes;

	OFFSHOP_DEBUG("lock request : guest %u , shop %u , item %u ",dwGuest, dwOwner, dwItem);

	if(m_offlineshopShopCache.LockSellItem(dwOwner, dwItem, valutes))
...
bool CClientManager::RecvOfflineShopCannotBuyLockItem(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopBuyItemPacket(CPeer* peer, const char* data)
{
	...
	auto it = pCache->itemsmap.find(dwItem);
	...
	if (!offerIDs.empty())
	{
		...
		for (auto offerID : offerIDs)
		{
			...
			if (m_offlineshopOfferCache.Get(offerID, &pOffer) && !pOffer->bAccepted)
			{
#ifdef __ENABLE_CHEQUE_SYSTEM__
				...
				SendOfflineShopOfferCancel(pOffer->dwOwnerID, offerID, true);
				m_offlineshopOfferCache.CancelOffer(offerID, pOffer->dwOwnerID);
			}
		}
	}
	...
	if(!m_offlineshopShopCache.SellItem(dwOwner, dwItem))
		sys_err("some problem with sell : owner_id(%u) item_id(%u)",dwOwner, dwItem);

	return true;
}

bool CClientManager::RecvOfflineShopEditItemPacket(CPeer* peer, const char* data)
{
	...
	if(m_offlineshopShopCache.EditItem(subpack->dwOwnerID, subpack->dwItemID, subpack->priceInfo))
		...
		sys_err("cannot edit item_id(%u) , shop owner_id(%u)",subpack->dwItemID , subpack->dwOwnerID);

	return true;
}

bool CClientManager::RecvOfflineShopRemoveItemPacket(CPeer* peer, const char* data)
{
	...
	{
		sys_err("cannot found shop owner %u into cache (item id %u)",subpack->dwOwnerID, subpack->dwItemID);
		return false;
	}
	...
	{
		sys_err("cannot found item id shop owner %u into cache (item id %u) ",subpack->dwOwnerID, subpack->dwItemID);
		return false;
	}
	...
	{
		// todo
		sys_err("cannot remove item owner_id(%u) item_id(%u)", subpack->dwOwnerID, subpack->dwItemID);
	}

	return true;
}

bool CClientManager::RecvOfflineShopAddItemPacket(CPeer* peer, const char* data)
{
	...
	if (!m_offlineshopShopCache.AddItem(subpack->dwOwnerID, item))
	{
		// todo
	}
	return true;
...
bool CClientManager::RecvOfflineShopForceClose(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopCreateNew(CPeer* peer, const char* data)
{
	...
	if (!m_offlineshopShopCache.CreateShop(subpack->shop.dwOwnerID, subpack->shop.dwDuration, subpack->shop.szName, vec, subpack->pos))
#else
	if (!m_offlineshopShopCache.CreateShop(subpack->shop.dwOwnerID, subpack->shop.dwDuration, subpack->shop.szName, vec))
#endif
	{
		// TODO
	}
...
bool CClientManager::RecvOfflineShopChangeName(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopOfferCreate(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopOfferNotified(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopOfferAccepted(CPeer* peer, const char* data)
{
	...
	if (!offerIDs.empty())
	{
		...
		for (auto offerID : offerIDs)
		{
			if(offerID == subpack->dwOfferID)
				continue;

			offlineshop::COfferCache::TOfferCacheInfo* pOfferToRemove = nullptr;
			if (m_offlineshopOfferCache.Get(offerID, &pOfferToRemove) && !pOfferToRemove->bAccepted)
			{
#ifdef __ENABLE_CHEQUE_SYSTEM__
				...
				SendOfflineShopOfferCancel(pOfferToRemove->dwOwnerID, offerID, true);
				m_offlineshopOfferCache.CancelOffer(offerID, pOfferToRemove->dwOwnerID);
			}
		}
	}
	...
	{
		auto it = pShop->itemsmap.find(pOffer->dwItemID);
		if (it != pShop->itemsmap.end())
		{
...
bool CClientManager::RecvOfflineshopOfferCancel(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopSafeboxGetItem(CPeer* peer, const char* data)
{
	...
	{
		sys_err("cannot withdraw item from safebox: owner_id(%u) item_id(%u)", subpack->dwOwnerID, subpack->dwItemID);

		// todo: send peer response
		return false;
	}
...
bool CClientManager::RecvOfflineShopSafeboxGetValutes(CPeer* peer, const char* data)
{
	...
	{
		sys_err("cannot withdraw valutes from safebox: owner_id(%u) gold(%u)", subpack->dwOwnerID, subpack->valute.illYang);
...
bool CClientManager::RecvOfflineShopSafeboxAddItem(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopAuctionCreate(CPeer* peer, const char* data)
{
...
bool CClientManager::RecvOfflineShopAuctionAddOffer(CPeer* peer, const char* data)
{
...
#endif
{
	...
	for (const auto& i : vec)
		OFFSHOP_DEBUG("item %u , vnum %u , count %u ", i.dwItemID, i.item.dwVnum, i.item.dwCount);
#endif
...
void CClientManager::SendOfflineshopSafeboxRemoveItem(DWORD dwOwnerID, DWORD dwItem)
{
	TPacketDGNewOfflineShop pack;
	pack.bSubHeader = offlineshop::SUBHEADER_DG_SAFEBOX_REMOVE_ITEM;

	offlineshop::TSubPacketDGSafeboxRemoveItem subpack;
	subpack.dwOwnerID = dwOwnerID;
	subpack.dwItemID = dwItem;
	...
	OFFSHOP_DEBUG("safebox: remove item id %u, %u owner", dwItem, dwOwnerID);
}
...
void CClientManager::OfflineShopResultSafeboxRemoveItemQuery(CQueryInfo* pQueryInfo)
{
	auto* qi = static_cast<offlineshop::SQueryInfoSafeboxRemoveItem*>(pQueryInfo->pvData);
	SendOfflineshopSafeboxRemoveItem(qi->dwOwnerID, qi->dwItemID);
	delete qi;
}
...
void CClientManager::OfflineShopResultQuery(CPeer* peer, SQLMsg* msg, CQueryInfo* pQueryInfo)
{
	...
	switch (pQueryInfo->iType)
	{
		...
		case QID_OFFLINESHOP_SAFEBOX_DELETE_ITEM:
			OfflineShopResultSafeboxRemoveItemQuery(pQueryInfo);
			break;
...
void CClientManager::OfflineshopExpiredShop(DWORD dwID)
{
	...
	if (m_offlineshopOfferCache.GetOffersByShopOwner(dwID, &pVec))
	{
		for (auto i : *pVec)
		{
			if (m_offlineshopOfferCache.Get(i, &pOffer) && !pOffer->bAccepted)
			{
	...
	if (!offerIDs.empty())
	{
		...
		for (auto offerID : offerIDs)
		{
			...
			if (m_offlineshopOfferCache.Get(offerID, &pOffer) && !pOffer->bAccepted)
			{
				...
				SendOfflineShopOfferCancel(pOffer->dwOwnerID, offerID, true);//offlineshop-updated 05/08/19
				m_offlineshopOfferCache.CancelOffer(offerID, pOffer->dwOwnerID);
			}
		}
	}
