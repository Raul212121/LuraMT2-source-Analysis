...
static constexpr int RANKING_DB_UPDATE = 60 * 30; // 30 min
#endif
...
void CClientManager::QUERY_SETUP(CPeer * peer, DWORD dwHandle, const char * c_pData)
{
	...
	for (DWORD c = 0; c < p->dwLoginCount; ++c, ++pck)
	{
		...
		r.discordUserId = pck->discordUserId;
...
void CClientManager::QUERY_AUTH_LOGIN(CPeer * pkPeer, DWORD dwHandle, TPacketGDAuthLogin * p)
{
	...
	else
	{
		...
		strlcpy(r.passwd, "TEMP", sizeof(r.passwd));
		r.discordUserId = p->discordUserId;

		sys_log(0, "AUTH_LOGIN id(%u) login(%s) social_id(%s) login_key(%u), client_key(%u %u %u %u) discordUserId(%llu)",
				p->dwID, p->szLogin, p->szSocialID, p->dwLoginKey,
				p->adwClientKey[0], p->adwClientKey[1], p->adwClientKey[2], p->adwClientKey[3], p->discordUserId);
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_REQUEST_CHANNELSTATUS:
				...
				break;
#ifdef ENABLE_RANKING_SYSTEM
			case HEADER_GD_RANKING_SYSTEM:
				RequestRanking(data, dwHandle, peer);
				break;
#endif
#ifdef ENABLE_EVENT_MANAGER
			...
			case HEADER_GD_UPDATE_AUTH_SHUTDOWN:
				UpdateAuthShutdown();
				break;
...
#ifdef ENABLE_RANKING_SYSTEM
		if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * RANKING_DB_UPDATE)))
		{
			UpdateRanking();
		}
#endif
...
bool CClientManager::__GetAdminInfo(const char *szIP, std::vector<tAdminInfo> & rAdminVec)
{
	char szQuery[512];
	snprintf(szQuery, sizeof(szQuery),
		"SELECT mID,mAccount,mName,mContactIP,mServerIP,mAuthority, mHideTeam FROM gmlist WHERE mServerIP='ALL' or mServerIP='%s'",
		szIP ? szIP : "ALL");
	...
	while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
	{
		...
		str_to_number(Info.m_bHideFromTeamList, row[idx++]);
...
void CClientManager::UpdateAuthShutdown()
{
	for (auto peer : m_peerList)
	{
		...
		peer->EncodeHeader(HEADER_DG_UPDATE_AUTH_SHUTDOWN, 0, 0);
	}
}
...
#ifdef ENABLE_RANKING_SYSTEM
void CClientManager::LoadRanking() {
	char szQuery[2048];
	snprintf(szQuery, sizeof(szQuery), "SELECT r.pid, r.monsters, r.metins, r.bosses, r.players, r.chests, p.name, pi.empire, g.name FROM rankings r "
		"LEFT JOIN player p on p.id = r.pid "
		"LEFT JOIN player_index pi on pi.id = p.account_id "
		"LEFT JOIN guild_member gm on gm.pid = r.pid "
		"LEFT JOIN guild g on g.id = gm.guild_id;");

	std::unique_ptr<SQLMsg> qData(CDBManager::Instance().DirectQuery(szQuery, SQL_PLAYER));


	if (qData->Get()->uiNumRows && qData->Get()->pSQLResult) {
		MYSQL_ROW row;
		while ((row = mysql_fetch_row(qData->Get()->pSQLResult))) {
			int i = 0;
			TRankingData data;
			str_to_number(data.pid, row[i++]);

			str_to_number(data.values[0], row[i++]);
			str_to_number(data.values[1], row[i++]);
			str_to_number(data.values[2], row[i++]);
			str_to_number(data.values[3], row[i++]);
			str_to_number(data.values[4], row[i++]);
			if (row[i])
				strncpy(data.name, row[i++], sizeof(data.name));
			else
				++i;
			str_to_number(data.empire, row[i++]);
			if (row[i])
				strncpy(data.guild, row[i++], sizeof(data.guild));
			else
				snprintf(data.guild, sizeof(data.guild), "-");
			m_RankingVector.insert({ data.pid, data });
		}
	}
}

void CClientManager::SaveRanking() {
	std::set<DWORD> emptyPids{};
	for (const auto& rank : m_RankingVector) {
		if (auto isEmpty = [&rank]() { return rank.second.values[CATEGORY_MONSTERS] == 0 && rank.second.values[CATEGORY_METINS] == 0 && rank.second.values[CATEGORY_BOSSES] == 0 && rank.second.values[CATEGORY_PLAYERS] == 0 && rank.second.values[CATEGORY_CHESTS] == 0; }; isEmpty())
		{
			emptyPids.emplace(rank.first);
			continue;
		}
		...
		snprintf(
			...
			"REPLACE INTO rankings(pid, monsters, metins, bosses, players, chests) VALUES (%d, %lld, %lld, %lld, %lld, %lld);",

			rank.first, rank.second.values[CATEGORY_MONSTERS],
			rank.second.values[CATEGORY_METINS],
			rank.second.values[CATEGORY_BOSSES],
			rank.second.values[CATEGORY_PLAYERS],
			rank.second.values[CATEGORY_CHESTS]
		);
		CDBManager::Instance().AsyncQuery(szQuery, SQL_PLAYER);
	}

	for (const auto pid : emptyPids)
	{
		m_RankingVector.erase(m_RankingVector.find(pid));
	}
}

void CClientManager::UpdateRanking() {
	SaveRanking();
}

void CClientManager::RequestRanking(const char* pData, DWORD dwHandle, CPeer* desc) {
	BYTE bSubHeader = *(BYTE*)pData;
	pData += sizeof(BYTE);
	switch (bSubHeader) {
	case 0:
	{
		BYTE bCategory = *(BYTE*)pData;
		pData += sizeof(BYTE);
		DWORD pid = *(DWORD*)pData;
		pData += sizeof(DWORD);
		std::vector<TRankingData> copyVector;
		for (const auto& rank : m_RankingVector)
			copyVector.push_back(rank.second);

		std::sort(copyVector.begin(), copyVector.end(), [bCategory](const TRankingData& X, const TRankingData& Y) -> bool
			{
				return X.values[bCategory] > Y.values[bCategory];
			});
		TRankingDataAnswer answer;
		answer.pid = pid;
		answer.count = 0;
		answer.rank = 0;
		answer.type = bCategory;
		bool bNeed = true;
		for (const auto& rank : copyVector)
		{
			if (answer.count == 10)
				break;

			answer.rankings[answer.count] = rank;
			memcpy(&answer.rankings[answer.count], &rank, sizeof(TRankingData));

			if (rank.pid == pid)
			{
				bNeed = false;
				answer.rank = answer.count;
			}
			++answer.rank;
			++answer.count;
		}
		...
		if (bNeed)
		{
			...
			for (const auto& rank : copyVector)
			{
				...
				if (rank.pid == pid)
				{
					answer.rankings[10] = rank;
					answer.rankings[10].selfPos = myRank;
					++answer.count;
					break;
				}
			}
		...
		desc->EncodeHeader(HEADER_DG_RANKING_SYSTEM, dwHandle, sizeof(TRankingDataAnswer));
		desc->Encode(&answer, sizeof(TRankingDataAnswer));
	} break;
	case 1: {
		DWORD bRankCount = *(DWORD*)pData;
		pData += sizeof(DWORD);

		for (DWORD i = 0; i < bRankCount; ++i) {
			TRankingData* bRank = (TRankingData*)pData;
			pData += sizeof(TRankingData);

			const auto& it = m_RankingVector.find(bRank->pid);
			if (it == m_RankingVector.end()) {
				m_RankingVector.insert({ bRank->pid, *bRank });
			}
			else {
				it->second.empire = bRank->empire;
				strcpy(it->second.name, bRank->name);
				strcpy(it->second.guild, bRank->guild);
				for (int j = 0; j < 20; ++j)
				{
					it->second.values[j] += bRank->values[j];
				}
			}
		}
	} break;
	default: {
		sys_err("RequestRanking - Unknown subheader %d", bSubHeader);
	} break;
	}
}
#endif
