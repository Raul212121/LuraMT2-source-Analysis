...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	void RecvGuildSymbolAuth(CPeer* pkPeer, DWORD dwHandle, const TPacketGuildSymbolAuthKey* packet);
    private:
	int					m_looping;
	socket_t				m_fdAccept;
...
public:
	...
	bool RecvOfflineShopBuyItemPacket(CPeer* peer, const char* data);
	bool RecvOfflineShopLockBuyItem(CPeer* peer, const char* data);
	bool RecvOfflineShopCannotBuyLockItem(CPeer* peer, const char* data);
	bool RecvOfflineShopEditItemPacket(CPeer* peer, const char* data);
	bool RecvOfflineShopRemoveItemPacket(CPeer* peer, const char* data);
	bool RecvOfflineShopAddItemPacket(CPeer* peer, const char* data);
	bool RecvOfflineShopForceClose(CPeer* peer, const char* data);
	bool RecvOfflineShopCreateNew(CPeer* peer, const char* data);
	bool RecvOfflineShopChangeName(CPeer* peer, const char* data);
	bool RecvOfflineShopOfferCreate(CPeer* peer, const char* data);
	bool RecvOfflineShopOfferNotified(CPeer* peer, const char* data);
	bool RecvOfflineShopOfferAccepted(CPeer* peer, const char* data);
	bool RecvOfflineshopOfferCancel(CPeer* peer, const char* data);
	bool RecvOfflineShopSafeboxGetItem(CPeer* peer, const char* data);
	bool RecvOfflineShopSafeboxGetValutes(CPeer* peer, const char* data);
	bool RecvOfflineShopSafeboxAddItem(CPeer* peer, const char* data);
	bool RecvOfflineShopAuctionCreate(CPeer* peer, const char* data);
	bool RecvOfflineShopAuctionAddOffer(CPeer* peer, const char* data);
	...
	void SendOfflineshopSafeboxRemoveItem(DWORD dwOwnerID, DWORD dwItem);
	...
	void OfflineShopResultSafeboxRemoveItemQuery(CQueryInfo* pQueryInfo);
...
private:
	void RequestRanking(const char* pData, DWORD dwHandle, CPeer* desc);
	void LoadRanking();
	void UpdateRanking();
	void SaveRanking();

private:
	std::unordered_map < DWORD, TRankingData > m_RankingVector;
	std::vector <TRankingData> m_RankingVectorYang;
#endif
...
#endif
public:
	void ExpiredLoginKey(const TPacketExpiredLoginKey* packet);
};
