...
void CGuild::Load(DWORD guild_id)
{
	...
	DBManager::instance().FuncQuery([this](SQLMsg* msg) { LoadGuildData(msg); },
	...
	DBManager::instance().FuncQuery([this](SQLMsg* msg) { LoadGuildGradeData(msg); },
			"SELECT grade, name, auth+0 FROM guild_grade%s WHERE guild_id = %u", get_table_postfix(), m_data.guild_id);

	DBManager::instance().FuncQuery([this](SQLMsg* msg) { LoadGuildMemberData(msg); },
...
{
	DBManager::instance().FuncQuery([this](SQLMsg* msg) { __P2PUpdateGrade(msg); }, "SELECT grade, name, auth+0 FROM guild_grade%s WHERE guild_id = %u and grade = %d", get_table_postfix(), m_data.guild_id, grade);
}
...
// {
    ...
    // if (m_guildPostCommentPulse > thecore_pulse()) {
        ...
        // ch->ChatPacket(CHAT_TYPE_INFO, "Poti posta un nou comentariu in: %02d minute si %02d secunde!", minutes, seconds);
        // return;
    // }
...
int CGuild::GetMaxMemberCount()
{
	...
	if (g_dwGuildMaxMembers > 0)
		return g_dwGuildMaxMembers;
...
void CGuild::ChangeGuildSymbol(LPCHARACTER ch)
{
	const auto desc = ch ? ch->GetDesc() : nullptr;
	if (!desc)
		return;

	const auto pid = ch->GetPlayerID();
	const auto member = GetMember(pid);
	if (!member)
		return;

	if (!HasGradeAuth(member->grade, GUILD_AUTH_NOTICE))
		return;

	if (GetLevel() < guild_mark_min_level)
		return;

	if ((thecore_pulse() - m_iLastChangeSymbolPulse) < PASSES_PER_SEC(10)) // 10 sec
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You can't change the symbol yet!");
		return;
	}

	m_iLastChangeSymbolPulse = thecore_pulse();

	TPacketGuildSymbolAuthKey packet{};
	packet.gid = GetID();
	packet.pid = pid;
	packet.empty = false;

	if (!SendGuildSymbolAuthData(desc, packet.key))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "You cannot change the symbol.");
		return;
	}

	db_clientdesc->DBPacket(HEADER_GD_GUILD_SYMBOL_AUTH_KEY, ch->GetDesc()->GetHandle(), &packet, sizeof(packet));
}

bool CGuild::SendGuildSymbolAuthData(LPDESC desc, uint8_t* key)
{
	if (desc == nullptr)
		return false;
	...
	randombytes_buf(sub_packet.nonce, sizeof(sub_packet.nonce));
	if (crypto_secretbox_easy(sub_packet.ciphertext, dummy, sizeof(dummy), sub_packet.nonce, key) != 0)
		return false;

	TPacketGCGuild packet{};
	packet.header = HEADER_GC_GUILD;
	packet.size = sizeof(packet) + sizeof(sub_packet);
	packet.subheader = GUILD_SUBHEADER_GC_SYMBOL_AUTH_DATA;

	TEMP_BUFFER buf;
	buf.write(&packet, sizeof(packet));
	buf.write(&sub_packet, sizeof(sub_packet));
	desc->Packet(buf.read_peek(), buf.size());

	return true;
}

void CGuild::ClearSymbolAuthKey(const DWORD pid)
{
	const auto member = GetMember(pid);
	if (!member)
		return;
	...
	memset(member->symbol_auth_key, 0, sizeof(member->symbol_auth_key));

	TPacketGuildSymbolAuthKey packet{};
	packet.gid = GetID();
	packet.pid = pid;
	packet.empty = true;
	db_clientdesc->DBPacket(HEADER_GD_GUILD_SYMBOL_AUTH_KEY, 0, &packet, sizeof(packet));
}
