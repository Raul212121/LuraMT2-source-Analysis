...
#ifdef ENABLE_BATTLE_PASS
	if (!isRewarded && m_map_attender.size() > 0)
	{
		for (const auto& player : m_map_attender)
		{
			auto pkChar = CHARACTER_MANAGER::instance().FindByPID(player.second);
			if (pkChar)
				pkChar->UpdateBattlePass(MISSION_FREE_OX, 1);
		}
		isRewarded = true;
	}
#endif
...
#endif
{
	...
	for (; iter != m_map_attender.end(); ++iter)
	{
		...
		if (pkChar)
		{
			...
			LogManager::instance().ItemLog(pkChar->GetPlayerID(), 0, count, dwItemVnum, "OXEVENT_REWARD", "", pkChar->GetDesc()->GetHostName(), dwItemVnum, pkChar->GetDiscordUserId());
		}
	}
