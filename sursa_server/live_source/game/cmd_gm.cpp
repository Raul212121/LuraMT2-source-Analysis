...
ACMD(do_item)
{
	...
	if (item)
	{
		...
		{
			int iEmptyPos = ch->GetEmptyPotionsInventory(item);
		...
		{
			int iEmptyPos = ch->GetEmptyBonusInventory(item);
...
ACMD(do_mob_map)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "%dx %s spawned", countSpawned, pkMob->m_table.szLocaleName);
}
...
ACMD(do_item_purge)
{
	...
	if (!*arg1)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, " special");
		return;
	}
	...
	if (!strArg.compare(0, 3, "all"))
	{
		...
		for (i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
		{
			if ((item = ch->GetItem(TItemPos(POTIONS_INVENTORY, i ))))
			{
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
			}
		}
		for (i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
		{
			if ((item = ch->GetItem(TItemPos(BONUS_INVENTORY, i ))))
			{
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
			}
		}
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   WARR:%3d%% ASAS:%3d%% SURA:%3d%% SHAM:%3d%% DEMI-HUMAN:%3d%%"
			,
...
ACMD(do_kill)
{
	...
	if (!tch)
	{
		...
		if (strcmp(arg1, "all") == 0)
		{
			...
			for (LPDESC d : c_set_desc)
			{
				if (d == nullptr || !d->IsPhase(PHASE_GAME))
					continue;
...
ACMD(do_block_chat)
{
	...
	if (tch && ch != tch)
		tch->AddAffect(AFFECT_BLOCK_CHAT, POINT_NONE, 0, AFF_NONE, lBlockDuration, 0, true);
    	tch->ChatPacket(CHAT_TYPE_NOTICE, "Ai primit mute pentru %d minute.", (lBlockDuration/60));
		char szNoticeMute[QUERY_MAX_LEN];
		if (lBlockDuration > 0)
			{
				snprintf(szNoticeMute, sizeof(szNoticeMute), "%s a primit mute pentru %ld minute de la %s.", tch->GetName(), (lBlockDuration/60), ch->GetName());
				BroadcastNotice(szNoticeMute);
			}
		else
		return;
}
...
ACMD(do_hardware_ban)
{
	...
	{
		if (ch)
			ch->ChatPacket(CHAT_TYPE_INFO, "Foloseste: /sugi_pula <nume>");
	...
	if (tch && tch->GetDesc())
	{
		std::unique_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT mac_addr FROM account.account WHERE id = %u", tch->GetDesc()->GetAccountTable().id));
		SQLResult* resSelect = pmsg->Get();
		if (resSelect && resSelect->uiNumRows > 0)
		{
			MYSQL_ROW row_hwid = mysql_fetch_row(resSelect->pSQLResult);
			if (strcmp(row_hwid[0], "not windows"))
			{
				DBManager::instance().Query("INSERT INTO account.banned_hwid_list VALUES('%s')", row_hwid[0]);
...
EVENTFUNC(shutdown_event)
{
	...
	{
		if (db_clientdesc != nullptr)
			db_clientdesc->DBPacketHeader(HEADER_GD_UPDATE_AUTH_SHUTDOWN, 0, 0);
	...
	{
		char buf[64];
		snprintf(buf, sizeof(buf), "Serverul se opreste pentru lucrari de intretinere in %d secunde.", *pSec);
		SendNotice(buf);
...
void Shutdown(int iSec)
{
	...
	{
		char buf[64];
		snprintf(buf, sizeof(buf), "Exista o intretinere a jocului in %d secunde.", iSec);
...
ACMD(do_maintenance)
{
	...
	if (!strcasecmp(arg1, "stop"))
	{
		...
		{
			if (ch)
				ch->ChatPacket(CHAT_TYPE_INFO, "Niciun eveniment de inchidere a serverului in curs.");
			return;
		}
	...
	{
		if (ch)
			ch->ChatPacket(CHAT_TYPE_INFO, "Usage: maintenance <timp> <durata min>(0 pentru dezactivare) (1m 30s)");
		return;
	}
...
#ifdef ENABLE_WORLD_BOSS
ACMD(do_world_force_spawn)
{
	if (ch->GetMapIndex() != 219)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Nu esti in mapa cu boss-ul");
		return;
	}
	...
	DWORD dwBoss = 0;
	str_to_number(dwBoss, arg1);

	CWorldBossManager::Instance().SpawnWorldBossCheck(dwBoss);
