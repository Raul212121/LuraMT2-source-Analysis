...
bool CPVPManager::Dead(LPCHARACTER pkChr, DWORD dwKillerPID, LPCHARACTER pkKiller)
{
	CPVPSetMap::iterator it = m_map_pkPVPSetByID.find(pkChr->GetPlayerID());
...
bool CAN_ATTACK_PLAYER_WHEN_RIDE_ZONE(long mapIndex)
{
	switch (mapIndex)
	{
	case 1:
...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if (pkVictim->IsGM() && pkVictim->IsAffectFlag(AFF_INVISIBILITY)) // by motz
		return false;
	...
	if( true == pkChr->IsHorseRiding() )
	{
		...
		if (pkChr->IsPC() && pkChr->GetPKMode() != PK_MODE_PEACE && pkVictim->IsPC())
		{
			if (!CAN_ATTACK_PLAYER_WHEN_RIDE_ZONE(pkChr->GetMapIndex()))
				return false;
		}
	}
	else
	{
		if (pkChr->GetMountVnum() != 0)
		{
			if (pkChr->IsPC() && pkChr->GetPKMode() != PK_MODE_PEACE && pkVictim->IsPC())
			{
				if (!CAN_ATTACK_PLAYER_WHEN_RIDE_ZONE(pkChr->GetMapIndex()))
	...
	if (pkChr->FindAffect(AFFECT_PK_PROTECT) || pkVictim->FindAffect(AFFECT_PK_PROTECT))
		return false;
