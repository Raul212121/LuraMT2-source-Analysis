...
CItem::CItem(DWORD dwVnum)
	...
	m_pkAccessorySocketExpireEvent(NULL), m_pkOwnershipEvent(NULL), m_dwOwnershipPID(0), m_bSkipSave(false),
	m_isLocked(false),
	m_dwMaskVnum(0), m_dwSIGVnum(0), m_bUpdateStatus(false)
{
...
#ifdef ENABLE_RENEWAL_ITEMS_PACKETS
...
LPITEM CItem::RemoveFromCharacter()
{
	...
	else
	{
		if (GetWindow() != SAFEBOX && GetWindow() != MALL)
		{
			...
			else if (IsPotions() && m_bWindow == POTIONS_INVENTORY)
			{
				if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
					sys_err("CItem::RemoveFromCharacter: pos >= SPECIAL_INVENTORY_MAX_NUM IsPotions");
				else
					pOwner->SetItem(TItemPos(POTIONS_INVENTORY, m_wCell), NULL);
			}
			else if (IsBonus() && m_bWindow == BONUS_INVENTORY)
			{
				if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
					sys_err("CItem::RemoveFromCharacter: pos >= SPECIAL_INVENTORY_MAX_NUM IsBonus");
				else
					pOwner->SetItem(TItemPos(BONUS_INVENTORY, m_wCell), NULL);
			}
			else if (IsChest() && m_bWindow == CHEST_INVENTORY)
			{
				if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
					sys_err("CItem::RemoveFromCharacter: pos >= SPECIAL_INVENTORY_MAX_NUM IsChest");
				else
					pOwner->SetItem(TItemPos(CHEST_INVENTORY, m_wCell), NULL);
			}
#endif
#ifdef ENABLE_SWITCHBOT
...
#ifdef ENABLE_SPECIAL_INVENTORY
	else if (UPGRADE_INVENTORY == window_type || POTIONS_INVENTORY == window_type || BONUS_INVENTORY == window_type || CHEST_INVENTORY == window_type)
	{
		if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
		{
			sys_err("CItem::AddToCharacter: cell overflow: %s to %s cell %d", m_pProto->szName, ch->GetName(), m_wCell);
...
#else
					toSetValue = (true == bAdd) ? GetVnum() : 0;
#endif
				}
...
#endif
		LogManager::instance().ItemLog(i, v, 0, GetID(), "SET_SOCKET", "", "", GetOriginalVnum(), GetOwner() ? GetOwner()->GetDiscordUserId() : 0);
	}
}
...
EVENTFUNC(unique_expire_event)
{
	...
	if (pkItem->GetValue(2) == 0)
	{
		...
		{
			sys_log(1, "UNIQUE_ITEM: expire %s %u", pkItem->GetName(), pkItem->GetID());

			LPCHARACTER owner = pkItem->GetOwner();
			if (owner != nullptr)
				owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s has expired."), pkItem->GetName());
...
EVENTFUNC(real_time_expire_event)
{
	...
	if (current > item->GetSocket(0))
	{
		...
		if (owner != nullptr)
			owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s has expired."), item->GetName());
...
void CItem::AttrLog()
{
	...
	for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
	{
		if (m_alSockets[i])
		{
			...
			if (g_iDbLogLevel>=LOG_LEVEL_MAX)
#endif
			LogManager::instance().ItemLog(i, m_alSockets[i], 0, GetID(), "INFO_SOCKET", "", pszIP ? pszIP : "", GetOriginalVnum(), GetOwner() ? GetOwner()->GetDiscordUserId() : 0);
		}
	}
	...
	for (int i = 0; i<ITEM_ATTRIBUTE_MAX_NUM; ++i)
	{
		...
		if (type)
		{
			...
			if (g_iDbLogLevel>=LOG_LEVEL_MAX)
#endif
			LogManager::instance().ItemLog(i, type, value, GetID(), "INFO_ATTR", "", pszIP ? pszIP : "", GetOriginalVnum(), GetOwner() ? GetOwner()->GetDiscordUserId() : 0);
		}
	}
}
...
bool CItem::IsUpgradeItem()
{
    ...
    DWORD denied_items[] = { 30179 };

    for (unsigned long denied_item : denied_items)
    {
       if (denied_item == GetVnum())
           return false;
    }

	for (unsigned long Item : Items)
	{
		if(Item == GetVnum())
			return true;
	}
	...
	if(GetType() == ITEM_MATERIAL && GetSubType() == ITEM_RESOURCE)
		return true;

	return false;
}
bool CItem::IsPotions()
{
    DWORD Items[] = {
		50821,50822,50823,50825,50826,70020,71101,
        71044, 71045, 27003, 27102, 71027, 71028, 71029, 71030,
        27006, 27105, 50817, 50818, 50819, 50820, 50801, 50813, 50814, 50802,
		50216, 50183, 27873, 27870, 50093, 27874, 25425, 25423,
		25422, 25424, 50094
	};
...
bool CItem::IsChest()
{
	...
	return (GetType() == ITEM_GIFTBOX || GetType() == ITEM_TREASURE_BOX || GetType() == ITEM_TREASURE_KEY);
}
#endif
