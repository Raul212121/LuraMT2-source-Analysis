...
bool IS_NO_CLEAR_ON_DEATH_AFFECT(uint32_t type) {
	...
	switch (type) {
		...
		case AFFECT_CRITICALA:
		case AFFECT_PATRUNDERE:
		case AFFECT_ZEU_DRAGON1:
		case AFFECT_ZEU_DRAGON2:
		case AFFECT_ZEU_DRAGON3:
		case AFFECT_ZEU_DRAGON4:
...
bool IS_NO_CLEAR_ON_MONSTER_DEATH_AFFECT(uint32_t type) {
	switch (type) {
		case SKILL_JEONGWI:
...
bool CHARACTER::UpdateAffect()
{
	if (!GetWear(WEAR_WEAPON))
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti activa Aura Sabiei fara o arma echipata."));
			RemoveAffect(SKILL_GEOMKYUNG);
		}

		if (IsAffectFlag(AFF_GWIGUM))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti activa Tais-ul Vrajit fara o arma echipata."));
...
void CHARACTER::ClearAffect(bool bSave, bool isDeadByMonster)
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		{
			if (isDeadByMonster &&
				IS_NO_CLEAR_ON_MONSTER_DEATH_AFFECT(pkAff->dwType)) {
				++it;
				continue;
			}

			if (IS_NO_CLEAR_ON_DEATH_AFFECT(pkAff->dwType) ||
				IS_NO_SAVE_AFFECT(pkAff->dwType, this)) {
				++it;
				continue;
			}
			...
			if (pkAff->dwType >= AFF_BIO_1 && pkAff->dwType <= AFF_BIO_11)
			{
				++it;
				continue;
			}
...
void CHARACTER::LoadAffect(DWORD dwCount, TPacketAffectElement * pElements)
{
	if (IsLoadedAffect())
	{
		sys_err("%s is trying to load affect again, hacker?", GetName());
		return;
	}
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	if (dwType == AFFECT_ANTIEXP && lDuration > 1)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu vei mai primi puncte de experienta."));
	}
#endif
...
#ifdef ENABLE_ANTIEXP_RING
	if (dwType == AFFECT_ANTIEXP)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Acum primesti puncte de experienta."));
	}
#endif
...
{
	RemovePoison(); //elimina otrava
	RemoveFire(); //elimina focul
	RemoveAffect(AFFECT_SLOW); //elimina incetinirea
	RemoveAffect(SKILL_PABEOB); //elimina risipa
}
