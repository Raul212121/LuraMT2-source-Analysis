...
namespace quest
{
    ...
    int pc_destroy_guild(lua_State * L)
    {
        ...
        if (g)
        {
            ...
            if(g->UnderAnyWar())
            {
                ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti distruge breasla cand esti in war!"));
                return 0;
            }
	...
	int pc_enough_inventory(lua_State* L)
	{
		...
		else if (pItem->IsPotions())
			bEnoughInventoryForItem = ch->GetEmptyPotionsInventory(pItem) != -1;
		else if (pItem->IsBonus())
			bEnoughInventoryForItem = ch->GetEmptyBonusInventory(pItem) != -1;
	...
	int pc_give_or_drop_item(lua_State* L)
	{
		...
		if (item && item->GetVnum() == 70015)
			ch->UpdateBattlePass(MISSION_FREE_COLLECT_ICE_CREAM, icount);
#endif
...
#endif
#ifdef ENABLE_BATTLE_PASS
	int pc_set_battle_pass(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
			return 0;
		...
		uint8_t vnum = (int)lua_tonumber(L, 1);
		uint64_t count = (int)lua_tonumber(L, 2);

		ch->UpdateBattlePass(vnum, count);
		return 1;
	}
#endif
...
#endif
#ifdef ENABLE_BATTLE_PASS
			{ "set_battle_pass", pc_set_battle_pass },
#endif
			{ NULL,			NULL			}
		};
