...
void CHARACTER::Initialize()
{
	...
	m_dwPlayerID = 0;
	m_dwKillerPID = 0;
	m_dwUseItemAntiFloodCount = 0;
	m_dwUseItemAntiFloodPulse = 0;
	m_iMoveCount = 0;
	...
	m_pointsInstant.playerSlots = std::make_unique<character_point_instant::PlayerSlotT>();
	...
	m_iSortInventoryTime = 0;
	memset(&m_iSortSpecialInventoryTime, 0, sizeof(m_iSortSpecialInventoryTime));
#endif
...
void CHARACTER::UpdatePacket()
{
	...
	pack2.dwGuildID = 0;
	pack2.sAlignment = 0;
#if defined(WJ_LEVEL_UPDATE_FIX)
...
void CHARACTER::FlushDelayedSaveItem()
{
	...
	{
		if ((item = GetUpgradeInventoryItem(i)))
			ITEM_MANAGER::instance().FlushDelayedSave(item);

		if ((item = GetPotionsInventoryItem(i)))
			ITEM_MANAGER::instance().FlushDelayedSave(item);

		if ((item = GetBonusInventoryItem(i)))
			ITEM_MANAGER::instance().FlushDelayedSave(item);

		if ((item = GetChestInventoryItem(i)))
			ITEM_MANAGER::instance().FlushDelayedSave(item);
	}
#endif
}
...
void CHARACTER::Disconnect(const char * c_pszReason)
{
	...
	if (m_pkExchange != nullptr) // by motz
	{
		m_pkExchange->Cancel();
		m_pkExchange = nullptr;
	}
...
bool CHARACTER::Show(long lMapIndex, long x, long y, long z, bool bShowSpawnMotion/* = false */)
{
	...
	else
	{
		ViewReencode();
		if (test_server)
			sys_log(0, "%s in same sectree", GetName());
	}
...
#ifdef ENABLE_RANKING_SYSTEM
	if (IsPC())
		CRankingSystem::Instance().UpdatePlayerInfo(this);
#endif
}
	...
	switch (type)
	{
		...
		case POINT_LEVEL:
			...
			if (GetGMLevel() != GM_IMPLEMENTOR)
			{
				switch (val)
				{
				case 75:	case 90:	case 105:
					char szNoticeLevelUp[QUERY_MAX_LEN];
					snprintf(szNoticeLevelUp, sizeof(szNoticeLevelUp), "Jucatorul %s a atins nivelul %u pe canalul %d!", GetName(), GetLevel(), g_bChannel);
		...
		case POINT_EXP:
			{
				...
				if (FindAffect(AFFECT_ANTIEXP) && amount > 0)
					return;
#endif
...
bool CHARACTER::SetSyncOwner(LPCHARACTER ch, bool bRemoveFromList)
{
	...
	else
	{
		...
		if (DISTANCE_APPROX(GetX() - ch->GetX(), GetY() - ch->GetY()) > 300)
		{
			sys_log(1, "SetSyncOwner distance over than 300 %s %s", GetName(), ch->GetName());
	...
	if (IsPC())
	{
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			if (GetMyShop())
			{
				...
				if (pkChrCauser->GetShop() && GetRaceNum() == 30000)
				{
...
#ifdef ENABLE_EVENT_FIND_NPC
	auto& eventInstance = EventManagerAddons::Instance();
	if (eventInstance.GetEventStatus() && this == eventInstance.GetEventNpc())
	{
		eventInstance.OnTriggerNPC(this, pkChrCauser);
		return;
	}
#endif
...
void CHARACTER::WarpEnd()
{
...

	{
		...
		p.discordUserId = GetDiscordUserId();
		P2P_MANAGER::instance().Send(&p, sizeof(TPacketGGLogin));
	}
}

bool CHARACTER::Return(bool regen_pos)
{
	if (!IsNPC())
		return false;
	...
	if (regen_pos && m_pkRegen != nullptr)
	{
		x = m_posRegen.x;
		y = m_posRegen.y;
	}
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
	...
	if (pkChr->IsPC())
	{
		...
		{
			// by motz
			if ((GetMobRank() == MOB_RANK_BOSS || GetMobRank() == MOB_RANK_KING) && pkChr->GetDungeon() == nullptr && m_pkRegen != nullptr)
			{
				if (DISTANCE_APPROX(m_posRegen.x - GetX(), m_posRegen.y - GetY()) >= 4800)
					return Return(true);
			}
...
void CHARACTER::LoadMall(int iItemCount, TPlayerItem * pItems)
{
	...
	sys_log(0, "%s mall load %d items", GetName(), iItemCount);
...
void CHARACTER::RevivePkProtected(int iDur)
{
    AddAffect(AFFECT_PK_PROTECT, POINT_NONE, 0, 0, iDur, 0, true);
}
...
void CHARACTER::SetBlockMode(BYTE bFlag)
{
	...
	SetQuestFlag("optiuni_joc.block_exchange", bFlag & BLOCK_EXCHANGE ? 1 : 0);
	SetQuestFlag("optiuni_joc.block_party_invite", bFlag & BLOCK_PARTY_INVITE ? 1 : 0);
	SetQuestFlag("optiuni_joc.block_guild_invite", bFlag & BLOCK_GUILD_INVITE ? 1 : 0);
	SetQuestFlag("optiuni_joc.block_whisper", bFlag & BLOCK_WHISPER ? 1 : 0);
	SetQuestFlag("optiuni_joc.block_messenger_invite", bFlag & BLOCK_MESSENGER_INVITE ? 1 : 0);
	SetQuestFlag("optiuni_joc.block_party_request", bFlag & BLOCK_PARTY_REQUEST ? 1 : 0);
}
...
namespace {
	...
	{
		public:
			FuncCheckWarp(LPCHARACTER pkWarp) : m_bUseWarp(false)
			{
				m_lTargetY = 0;
				m_lTargetX = 0;
...
bool CHARACTER::WarpToPID(DWORD dwPID, bool checkWarp)
{
	if (checkWarp)
	{
		if (!CanWarp())
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot be teleported right now."));
			return false;
		}
	}
...
#ifdef ENABLE_SORT_INVENTORY
	if ((iPulse - GetSortInventoryTime()) < PASSES_PER_SEC(limittime))
	{
		return true;
	}

#ifdef ENABLE_SPECIAL_INVENTORY
	for (BYTE i = 0; i < SORT_INVENTORY_MAX_NUM; ++i) {
		if ((iPulse - GetSortSpecialInventoryTime(i)) < PASSES_PER_SEC(limittime))
		{
			return true;
		}
	}
...
#ifdef ENABLE_SORT_INVENTORY
	if ((iPulse - GetSortInventoryTime()) < limit_time)
		return false;

#ifdef ENABLE_SPECIAL_INVENTORY
	for (BYTE i = 0; i < SORT_INVENTORY_MAX_NUM; ++i) {
		if ((iPulse - GetSortSpecialInventoryTime(i)) < limit_time)
			return false;
	}
#endif
#endif
...
EVENTFUNC(move_channel)
{
	...
	if (info->iSec > 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Schimbi canalul in %d."), info->iSec);
		--info->iSec;
...
{
	if (IsHack(false, true, 3))
		return false;

	move_channel_info* info = AllocEventInfo<move_channel_info>();
	info->ch = this;
	info->iSec = CanWarp() && !IsPosition(POS_FIGHTING) ? 0 : 3;
...
void CHARACTER::MountSummon(LPITEM mountItem)
{
	...
	if (IsPolymorphed())
	{
		ChatPacket(CHAT_TYPE_INFO, "Aceasta operatiune nu este permisa in timpul transformarii.");
		return;
	}
...
void CHARACTER::CheckMount()
{
	...
	LPITEM armorItem = GetWear(WEAR_BODY);
	DWORD mobVnum = 0;
	...
	if (armorItem && (armorItem->GetVnum() >= 11901 && armorItem->GetVnum() <= 11904))
	{
...
std::string CHARACTER::GetHardwareID()
{
	...
	char szQuery[1024];
	snprintf(szQuery, sizeof(szQuery), "SELECT machine_guid FROM account.account WHERE id = '%d' LIMIT 1", GetDesc()->GetAccountTable().id);
...
void CHARACTER::SendDropLettersItem()
{
	const CSpecialItemGroup* pGroup = ITEM_MANAGER::instance().GetSpecialItemGroup(BOX_VNUM_L);

	if (pGroup)
	{
		for (int i = 0; i < pGroup->GetGroupSize(); i++)
		{
			ChatPacket(CHAT_TYPE_COMMAND, "letters_drop %d %d %d %d", pGroup->GetVnum(i), pGroup->GetCount(i), i, pGroup->GetGroupSize());
		}
	}
}
#endif
...
void CHARACTER::SendRewardHourData()
{
	if (GetDesc())
	{
		TPacketRewardHours packet = {};
		packet.bHeader = HEADER_GC_REWARD_HOURS;
		packet.stage = GetQuestFlag("hour.collect");

		GetDesc()->Packet(&packet, sizeof(TPacketRewardHours));
	}
}
...
bool CHARACTER::CanTakeInventoryItem(LPITEM item, TItemPos* cell)
{
	...
	{
		cell->window_type = POTIONS_INVENTORY;
		cell->cell = iEmpty = GetEmptyPotionsInventory(item);
	}
	else if (item->IsBonus())
	{
		cell->window_type = BONUS_INVENTORY;
		cell->cell = iEmpty = GetEmptyBonusInventory(item);
	}
...
#endif
#ifdef ENABLE_BATTLE_PASS
void CHARACTER::GetBattlePass()
{
	battlepass.clear();

	char szQuery[1024];
	snprintf(szQuery, sizeof(szQuery), "SELECT mission, progress, iscompleted, isActive FROM battlepass WHERE pid = %u;", GetPlayerID());
	std::unique_ptr<SQLMsg> pkPrint(DBManager::instance().DirectQuery(szQuery));

	auto pRes = pkPrint->Get();
	MYSQL_ROW data;
	...
	if (pRes && pRes->uiNumRows > 0)
	{
		while ((data = mysql_fetch_row(pRes->pSQLResult)))
		{
			...
			bp.missionVnum = std::stoi(data[BP_PID]);
			bp.progress = std::stoi(data[BP_PROGRESS]);
			bp.isCompleted = std::stoi(data[BP_DONE]);
			bp.isActive = std::stoi(data[BP_ACTIVE]);

			battlepass.insert(std::make_pair(bp.missionVnum, bp));
		}
	}
}
void CHARACTER::DestroyBattlePass()
{
	if(!IsPC())
		return;

	DBManager::instance().Query("DELETE FROM battlepass WHERE pid = %u", GetPlayerID());
	for (const auto& items : battlepass)
	{
		char szQuery[1024];
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO battlepass (pid, mission, progress, iscompleted, isActive) VALUES ( %d, %d, %d, %d, %d);",
			GetPlayerID(), items.first, items.second.progress, items.second.isCompleted, items.second.isActive);

		DBManager::instance().Query(szQuery);
	}
}

void CHARACTER::UnlockBattlePass(uint16_t id)
{
	TBattlePass bp;

	auto&& [start, end] = CBattlePass::instance().GetBattlePassRange(id);
	for (auto i = start; i <= end; i++)
	{
		auto it = battlepass.find(i);
		if (it == battlepass.end())
		{
			...
			battlepass.insert(std::make_pair(i, bp));
		}
	}
	BattePassLogin();
}

void CHARACTER::UpdateBattlePass(uint8_t mission, int64_t value)
{
	if (mission >= MISSION_BP_LAST_NUM)
	{
		sys_err("Wrong BattlePass Mission, vnum %d, pkChr %s", mission, GetName());
		return;
	}

	if (IsBattlePassFinished(mission))
		return;

	auto it = battlepass.find(mission);
	if (it != battlepass.end())
	{
		if (it->second.isCompleted)
			return;

		const auto& passTable = CBattlePass::instance().GetBattlePassMap(mission);
		if (!passTable)
		{
			sys_err("Failed to init battlepass table, mission %d", mission);
			return;
		}
		if (it->second.progress + value >= passTable->maxProgress)
		{
			it->second.progress = passTable->maxProgress;
			it->second.isCompleted = true;

			for (auto items = 0; items < BATTLEPASS_REWARD_COUNT; items++)
				AutoGiveItem(passTable->rewards[items].vnum, passTable->rewards[items].count);

			if (IsBattlePassFinished(mission))
				RewardFinalBattlePass(GetBattleMission(mission));

		}
		else it->second.progress = it->second.progress + value;

		if (GetDesc())
		{
			TEMP_BUFFER buf;
			TPacketBattlePass packet;
			TPacketSubBattlePassUpdate sub;

			packet.bHeader = HEADER_GC_BATTLE_PASS;
			packet.wSize = sizeof(TPacketBattlePass) + sizeof(TPacketSubBattlePassUpdate);
			packet.bSubHeader = BATTLEPASS_GC_SUB_UPDATE;

			sub.missionVnum = mission;
			sub.progress = it->second.progress;
			sub.isActive = it->second.isActive;

			buf.write(&packet, sizeof(TPacketBattlePass));
			buf.write(&sub, sizeof(TPacketSubBattlePassUpdate));

			GetDesc()->Packet(buf.read_peek(), buf.size());
		}

		if (test_server) ChatPacket(CHAT_TYPE_INFO, "Battlepass Progress %d / %d", it->second.progress, passTable->maxProgress);
	}
}
void CHARACTER::SkipBattleMission(uint8_t mission)
{
	if (IsBattlePassFinished(mission))
		return;
...
uint8_t CHARACTER::GetBattleMission(uint8_t id)
{
	if (id >= MISSION_FREE_BOSS_SLAYER_1 && id <= MISSION_FREE_END)
		return BATTLEPASS_FREE;
	return 0;
}

bool CHARACTER::IsBattlePassFinished(uint8_t id)
{
	uint8_t battleVnum = GetBattleMission(id);

	const auto& [start, end] = CBattlePass::instance().GetBattlePassRange(battleVnum);
	for (auto i = start; i < end; i++)
	{
		auto it = battlepass.find(i);
		if (it != battlepass.end())
...
void CHARACTER::BattePassLogin()
{
	...
	packet.bHeader = HEADER_GC_BATTLE_PASS;
	packet.bSubHeader = BATTLEPASS_GC_SUB_INIT;

	for (const auto& dataSend : battlepass)
		buf.write(&dataSend.second, sizeof(dataSend.second));

	packet.wSize = sizeof(packet) + buf.size();

	if (buf.size())
	{
		GetDesc()->BufferedPacket(&packet, sizeof(TPacketBattlePass));
		GetDesc()->Packet(buf.read_peek(), buf.size());
	}
	else GetDesc()->Packet(&packet, sizeof(TPacketBattlePass));
}
void CHARACTER::RewardFinalBattlePass(uint8_t mission)
{
	const auto& rewardMap = CBattlePass::instance().GetRewardMap();
	const auto& rewardMapPtr = rewardMap.find(mission);
	if (rewardMapPtr == rewardMap.end())
	{
		sys_err("[BattlePass] Failed to get final reward, user %s, pass %d", GetName(), mission);
		return;
	}

	for (const auto& item : rewardMapPtr->second)
		AutoGiveItem(item.vnum, item.count);

}
#endif

bool CHARACTER::IsShootSpeedHack(const uint8_t seconds, const uint8_t count)
{
	if (const auto pulse = thecore_pulse(); pulse > static_cast<int32_t>(GetShootSpeedPulse() + PASSES_PER_SEC(seconds)))
	{
		SetShootSpeedCount(0);
		SetShootSpeedPulse(pulse);
	}
	return IncreaseShootSpeedCount() >= count;
}
