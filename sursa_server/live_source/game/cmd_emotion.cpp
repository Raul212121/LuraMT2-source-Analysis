...
ACMD(do_emotion)
{
	...
	bool bAttack = (get_dword_time() - ch->GetLastMoveTime()) < 750;

	if (bMoving || bAttack)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Trebuie sa stai pe loc pentru a utiliza emotia."));
		return;
	}
	...
	{
		if (strcmp(cmd_info[cmd].command, emotion_types[i].command) == 0)
			break;

		if (strcmp(cmd_info[cmd].command, emotion_types[i].command_to_client) == 0)
			break;
	}
	...
	if (victim)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Unblock %s to continue."), victim->GetName());
			return;
		}

		if (MessengerManager::instance().IsBlocked(victim->GetName(), ch->GetName()))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s has blocked you."), victim->GetName());
			return;
		}
#endif
		...
		{
			if (ch->GetGMLevel() != GM_IMPLEMENTOR) { // GAY STYLE
