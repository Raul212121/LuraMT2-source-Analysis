...
void CShopManager::Sell(LPCHARACTER ch, WORD wCell, BYTE bCount, BYTE byType)
#else
...
#endif
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot sell the item if you have opened a private shop."));
		return;
	}
#endif
...
#ifdef ENABLE_SPECIAL_INVENTORY
	LPITEM item = ch->GetItem(TItemPos(byType, wCell));
#else
	...
	const unsigned long long nTotalMoney = static_cast<unsigned long long>(ch->GetGold()) + static_cast<unsigned long long>(dwPrice);
