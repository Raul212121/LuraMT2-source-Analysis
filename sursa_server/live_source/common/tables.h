...
enum
{
	...
	HEADER_GD_UPDATE_AUTH_SHUTDOWN = 111,
	...
	HEADER_GD_NEW_OFFLINESHOP = 141,
#endif

#ifdef ENABLE_RANKING_SYSTEM
	HEADER_GD_RANKING_SYSTEM,
#endif
#ifdef ENABLE_EVENT_MANAGER
	HEADER_GD_EVENT_MANAGER,
#endif
	HEADER_GD_GUILD_SYMBOL_AUTH_KEY,
	...
	HEADER_DG_UPDATE_AUTH_SHUTDOWN = 143,
...
#ifdef ENABLE_RANKING_SYSTEM
	HEADER_DG_RANKING_SYSTEM,
#endif
#ifdef ENABLE_EVENT_MANAGER
	HEADER_DG_EVENT_MANAGER,
#endif
	HEADER_DG_GUILD_SYMBOL_AUTH,
	HEADER_DG_GUILD_SYMBOL_AUTH_KEY,
...
typedef struct TAdminInfo
{
	...
	int m_Authority;
	bool m_bHideFromTeamList;
	uint8_t restrict_flag;
} tAdminInfo;
//END_ADMIN_MANAGER
...
typedef struct rewardHours
{
	std::vector<TRewards> rewards;
	uint32_t minutes;
}TRewardHour;
...
namespace offlineshop
{
	...
	enum eSubHeaderDGNewOfflineshop
	{
		...
		SUBHEADER_DG_SAFEBOX_REMOVE_ITEM,
	};
	...
	typedef struct SSubPacketDGSafeboxRemoveItem
	{
		DWORD dwOwnerID, dwItemID;
	} TSubPacketDGSafeboxRemoveItem;

	typedef struct SSubPacketDGSafeboxRemoveValutes
	{
		DWORD dwOwnerID;
		TValutesInfo valute;
	} TSubPacketDGSafeboxRemoveValutes;
...
enum
{
	...
	BATTLEPASS_REWARD_COUNT_FINAL = 7,
};
...
{
	uint8_t missionVnum = 0;
	int32_t progress = 0;

	uint8_t isCompleted = false;
	bool isActive = false;

} TBattlePass;

typedef std::map<uint32_t, TBattlePass> TPassMap;

typedef struct BattleNetwork
{
...
//Network
typedef struct BattlePassNetwork
{
	int16_t player;
...
typedef struct SRankingDataAnswer {
	DWORD pid;
	BYTE type;
	BYTE count;
	DWORD rank;
	TRankingData rankings[10];
	TRankingData selfRank;
} TRankingDataAnswer;
#endif
#ifdef ENABLE_EVENT_MANAGER
