...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		if(dwRecvCount++ >= MAX_RECV_COUNT-1 && GetRecvBufferSize() < SAFE_RECV_BUFSIZE
			&& m_strPhase == "Game") //phase_game ?? 取芋╞?取??? ０?▽?﹞? ??取?０?╞? ﹉屆０?﹉╳ ??╞?.
			break;
		...
		switch (header)
		{
			...
			case HEADER_GC_PHASE:
				ret = RecvPhasePacket();
				return; // ???帕０╳ Phase ﹉╳ 易?昌?╱? ??╞? 易??Ｄ﹉? GamePhase ???? - [levites]
				break;
...
void CPythonNetworkStream::Warp(LONG lGlobalX, LONG lGlobalY)
{
	...
	// NOTE : Warp ?帕??Ｄ＝ CenterPosition?? Height﹉╳ 0??▽? Ｄ＝易Ｙ０╳ ??╱身Ｄ?﹉╳ Ｄ?易?╞?０╳ 易?????﹉? ??
	//        ０??㊣??Ｄ＝╱Ｄ╞? Height﹉╳ ﹉?卦? ??▽? Ｄ＝易Ｙ??易?﹞? ╱??? ???０??╱? Position?? ﹉〝??﹞? ??易帚
	//        卹?芋??赤?赤╞? - [levites]
...
bool CPythonNetworkStream::SendMessengerAddByNamePacket(const char * c_szName)
{
	...
	szName[CHARACTER_NAME_MAX_LEN-1] = '\0'; // #720: ╱身卦??? ??╱＝ ﹉?﹞? 易?芋? ０?易???﹞?０? 易?▽▼ 卹??５
...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	// ??﹞╞ 取芋Ｄ帚 易??? ?昆╱Ｙ
	// "??╱＝: 昆?０?" ??﹞??? "昆?０?: ??╱＝" 卹帚卹〝﹞? ??﹞???▽? ?＝?赤 ??(0x08)?? 昆??卦
	// ???? 取芋Ｄ帚取? ▽???﹞? ?昆╱Ｙ?赤 (０?取?1) : (０?取?2) ﹞? ??﹞???取??? (０?取?2) : (０?取?1) ﹞? ??﹞???﹉? ╱╱??╞?
	...
	if (kChat.dwVID != 0)
	{
		...
		switch (kChat.type)
		{
		case CHAT_TYPE_TALKING:  /* ▽▼昆? ?５芋? */
		case CHAT_TYPE_PARTY:    /* 芋?芋卹╱? */
		case CHAT_TYPE_GUILD:    /* ▽屆??╱? */
		case CHAT_TYPE_SHOUT:	/* ０??╳▽? */
		case CHAT_TYPE_WHISPER:	// 卹〝易?０?╞? ０??０???? 取?╞? Only Client Enum
			{
				char * p = strchr(buf, ':');
		...
		case CHAT_TYPE_INFO:     /* ?５足╱ (取芋?????? ??取?╞?, ﹉屆???╳╱? 取?取?╞?. ??) */
		case CHAT_TYPE_NOTICE:   /* ﹉帚?????▼ */
...
{
	assert(!"CPythonNetworkStream::RecvSkillLevel - ??０????? 取?╞? ??卹?");
...
bool CPythonNetworkStream::RecvPartyUpdate()
{
	...
	// ╱╱取? ╱Ｙ╞?﹉╳ 易?昌?取?╞?╱?, TargetBoard ?? 易?芋﹉?? 取㊣????芋Ｙ ??╞?.
	DWORD dwVID;
...
bool CPythonNetworkStream::SendClientVersionPacket()
{
	...
	if (LocaleService_IsEUROPE() && false == LocaleService_IsYMIR())
	{
		...
		strncpy(kVersionPacket.timestamp, "1215955205", sizeof(kVersionPacket.timestamp)-1); // # python time.time 取???╱Ｙ
...
//#ifdef _DEBUG
//	char szBuf[256+1];
//	_snprintf(szBuf, sizeof(szBuf), "?╱﹉??? ??卹足 ??取?卦?╞?╞? [%d:%s]", kTargetCreate.lID, kTargetCreate.szTargetName);
...
//#ifdef _DEBUG
//	char szBuf[256+1];
//	_snprintf(szBuf, sizeof(szBuf), "?昆╱??? ?╱﹉??? ??卹足 ??取?卦?╞?╞? [%d:%s:%d]", kTargetCreate.lID, kTargetCreate.szTargetName, kTargetCreate.dwVID);
//	CPythonChat::Instance().AppendChat(CHAT_TYPE_NOTICE, szBuf);
...
//#ifdef _DEBUG
//	char szBuf[256+1];
//	_snprintf(szBuf, sizeof(szBuf), "?╱﹉??? ?＝?╳﹉╳ ﹉?卦? ??取?卦?╞?╞? [%d:%d/%d]", kTargetUpdate.lID, kTargetUpdate.lX, kTargetUpdate.lY);
//	CPythonChat::Instance().AppendChat(CHAT_TYPE_NOTICE, szBuf);
...
static constexpr auto DiscordClientID = "1465509677033980128";
auto szSiteName = "www.luramt2.ro";
auto szIconNamelogin = "login";
auto szIconNameselect = "select";
auto szIconNameloading = "loading";
auto szIconName = "image";
...
void CPythonNetworkStream::Discord_Update(const bool login, const bool select, const bool loading)
{
	...
	if (login)
	{
		discordPresence.details = "Meniu LogIn";
		discordPresence.state = szSiteName;
		discordPresence.largeImageKey = szIconNamelogin;
		discordPresence.largeImageText = szSiteName;
	}
	else if (select)
	{
		discordPresence.details = "Selectare Erou";
		discordPresence.state = szSiteName;
		discordPresence.largeImageKey = szIconNameselect;
		discordPresence.largeImageText = szSiteName;
	}
	else if (loading)
	{
		...
		discordPresence.state = szSiteName;
		discordPresence.largeImageKey = szIconNameloading;
		discordPresence.largeImageText = szSiteName;
	}
	else
	{
		...
		if (GuildID != 0)
			_snprintf(szBufName, sizeof(szBufName), " %s -  %s", CPythonPlayer::Instance().GetName(), &GuildName);
		else
			_snprintf(szBufName, sizeof(szBufName), " %s", CPythonPlayer::Instance().GetName());
