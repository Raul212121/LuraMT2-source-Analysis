...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	char securityKey[32 + 1];
	strlcpy(securityKey, pinfo->securityKey, sizeof(securityKey));
	
	std::string g_stSecurityKey = EXTRA_LOGIN_SECURITY_KEY; //  EXTRA_LOGIN_SECURITY_KEY already defined in service.h
	
	if (0 != g_stSecurityKey.compare(securityKey)) // there compare code from client and server , if that will be false :
	{
		LoginFailure(d, "VERSION");     // that will be in intrologin.py , Python / ROOT     
		return;							// return , nothing , all actions will be blocked imediatly. Easy.
	}
#endif
