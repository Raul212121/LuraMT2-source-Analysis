...
const uint32_t PICKUP_DISTANCE = 3 * 1000 * 1000;
...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, float x, float y, float z, DWORD dwCount, bool bDrop, long alSockets[ITEM_SOCKET_SLOT_MAX_NUM], TPlayerItemAttribute aAttrs[ITEM_ATTRIBUTE_SLOT_MAX_NUM])
#else
...
#endif
{
	...
	static char itemCount[256];
	sprintf_s(itemCount, " (%d)", dwCount);
	...
	case CItemData::ITEM_TYPE_SKILLFORGET:
	{
		...
		if ((dwSkillVnum != 0) && CPythonSkill::Instance().GetSkillData(dwSkillVnum, &c_pSkillData))
			len += snprintf(szItemName, sizeof(szItemName), "%s %s", c_pSkillData->GradeData[0].strName.c_str(), (dwCount > 1) ? itemCount : "");
...
bool CPythonItem::GetCloseItem(const TPixelPosition& c_rPixelPosition,
	...
	DWORD dwDistance) {
	DWORD dwCloseItemID = 0;
	DWORD dwCloseItemDistance = PICKUP_DISTANCE;

	TGroundItemInstanceMap::iterator i;
	for (i = m_GroundItemInstanceMap.begin();
		i != m_GroundItemInstanceMap.end(); ++i) {
		TGroundItemInstance* pInstance = i->second;

#ifdef ENABLE_PICKUP_FILTER
		if (IsIgnoredItem(pInstance->dwVirtualNumber)) {
			continue;
		}
#endif

		auto dx = static_cast<int32_t>(c_rPixelPosition.x) -
			static_cast<int32_t>(pInstance->v3EndPosition.x);
		auto dy = static_cast<int32_t>(c_rPixelPosition.y) -
			(-static_cast<int32_t>(pInstance->v3EndPosition.y));

		auto distance = dx * dx + dy * dy;

		if (distance < dwCloseItemDistance) {
			dwCloseItemID = i->first;
			dwCloseItemDistance = dwDistance;
		}
	}
...
bool CPythonItem::IsIgnoredItem(DWORD dwVirtualNumber) {
	CItemData* pkItemData;
	if (!CItemManager::instance().GetItemDataPointer(dwVirtualNumber,
		&pkItemData)) {
		return true;
	}

	bool ignore_pickup_weapon = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_WEAPON);
	bool ignore_pickup_armor = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_ARMOR);
	bool ignore_pickup_head = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_HEAD);
	bool ignore_pickup_shield = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_SHIELD);
	bool ignore_pickup_wrist = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_WRIST);
	bool ignore_pickup_foots = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_FOOTS);
	bool ignore_pickup_neck = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_NECK);
	bool ignore_pickup_ear = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_EAR);
	bool ignore_pickup_etc = CPythonSystem::Instance().GetPickupIgnore(
		CPythonSystem::PICKUP_IGNORE_ETC);

	if (pkItemData->GetType() == CItemData::ITEM_TYPE_WEAPON &&
		ignore_pickup_weapon) {
		return true;
	}

	if (pkItemData->GetType() == CItemData::ITEM_TYPE_ARMOR) {
		bool ignored = false;
	...
	if ((pkItemData->GetType() != CItemData::ITEM_TYPE_WEAPON &&
		pkItemData->GetType() != CItemData::ITEM_TYPE_ARMOR) &&
		ignore_pickup_etc) {
		return true;
	}
...
std::vector<DWORD> CPythonItem::GetCloseMoneys(
	const TPixelPosition& c_rPixelPosition, DWORD dwDistance) {
	std::vector<DWORD> vecCloseItems;
	DWORD dwCloseItemID = 0;
	...
	for (i = m_GroundItemInstanceMap.begin();
		...
		if (distance < PICKUP_DISTANCE) {
			vecCloseItems.push_back(i->first);
		}
	}

	return vecCloseItems;
}

std::vector<DWORD> CPythonItem::GetCloseItems(
	const TPixelPosition& c_rPixelPosition, DWORD dwDistance) {
	std::vector<DWORD> vecCloseItems;
	DWORD dwCloseItemID = 0;
