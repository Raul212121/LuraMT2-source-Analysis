...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	public:
		...
		typedef struct SPlayerStatus
		{
			...
			TItemData			aPotItem[c_Special_ItemSlot_Count];
			TItemData			aBnsItem[c_Special_ItemSlot_Count];
			TItemData			aSCSItem[c_Special_ItemSlot_Count];
#endif	
	...
	protected:
		...
		void	__OnPressSmart(CInstanceBase& rkInstMain, bool isAuto, bool skipActor = false);
		void	__OnClickSmart(CInstanceBase& rkInstMain, bool isAuto, bool skipActor = false);
	...
	public:
		void SetHoursStage(uint16_t hours) { hoursStage = hours; }
		auto GetHoursStage() { return hoursStage; }
	private:
		uint16_t hoursStage = 0;
};
