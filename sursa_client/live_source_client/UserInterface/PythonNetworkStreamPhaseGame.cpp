...
void CPythonNetworkStream::GamePhase()
{
	...
	const DWORD SAFE_RECV_BUFSIZE = 10240;
	DWORD dwRecvCount = 0;
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_RANKING_SYSTEM:
				ret = RecvRankingSystem();
				break;
#endif
			...
			case HEADER_GC_SEND_BIOLOG_INFO:
				ret = RecvBiologInfo();
				break;
#endif
			...
			case HEADER_GC_TARGET_INFO_DROP:
				ret = RecvTargetInfoDrop();
				break;
#endif
			default:
...
bool CPythonNetworkStream::RecvWhisperPacket()
{
	...
	{
		if (strcmp(whisperPacket.szNameFrom, "[Switchbot]") == 0)
		{
#ifdef ENABLE_WHISPER_FLASHING
			FLASHWINFO flashInfo = {};
			flashInfo.cbSize = sizeof(flashInfo);
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			if (pInstTarget->IsPC() || pInstTarget->IsBuilding())
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetPCTargetBoard", Py_BuildValue("(isiii)", TargetPacket.dwVID, pInstTarget->GetNameString(), TargetPacket.bHPPercent, TargetPacket.iMinHP, TargetPacket.iMaxHP));
...
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char* c_szName, const std::vector<TShopItemTable>& c_rSellingItemStock)
{
	...
	packet.bCount = (uint8_t)c_rSellingItemStock.size();
...
auto szSiteName = "";
auto szImageIcon = "image1";

void CPythonNetworkStream::SetDiscordUserId(const uint64_t userId)
{
	m_DiscordUserId = userId;
}

void CPythonNetworkStream::DiscordRunCallbacks()
{
...
void CPythonNetworkStream::Discord_Start()
{
	...
	handlers.ready = [](const DiscordUser* discordUser) { CPythonNetworkStream::Instance().SetDiscordUserId(std::stoull(discordUser->userId)); };
...
void CPythonNetworkStream::Discord_Update(const bool ingame)
{
	...
	discordPresence.largeImageKey = szImageIcon;
	...
	//discordPresence.buttonLabel = "Join LuraMT2";
	//discordPresence.buttonURL = "https://discord.gg/YTHzZRqbuN";
...
#ifdef ENABLE_BIOLOG_SYSTEM
bool CPythonNetworkStream::RecvBiologInfo()
{
	TPacketGCSendInfoBiolog packet;
	if (!Recv(sizeof(packet), &packet))
	{
		Tracef("Recv RecvBiologInfo Packet Error\n");
		return false;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BIOLOG_TIME", Py_BuildValue("(i)", packet.iTimeLeft));

	if (packet.bInfoChecked == false)
		return true;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BIOLOG", Py_BuildValue("(iiiii)", packet.iRewardItem, packet.iReqItem[0], packet.iReqItem[1], packet.iReqItem[2], packet.iReqLevel));
	for (int i = 0; i < 8; ++i)
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BIOLOG_REWARD_BONUS", Py_BuildValue("(i)", packet.iRewardPoints[i]));
	
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BIOLOG_DONE", Py_BuildValue("()"));
...
#ifdef ENABLE_SHOW_CHEST_DROP
bool CPythonNetworkStream::SendChestDropInfo(DWORD itemvnum)
{
	TPacketCGChestDropInfo packet;
	packet.header = HEADER_CG_CHEST_DROP_INFO;
	packet.itemvnum = itemvnum;
...
#ifdef ENABLE_BATTLE_PASS
bool CPythonNetworkStream::RecvBattlePass()
{
	...
	case BATTLEPASS_GC_SUB_INIT:
	{
		...
		while (packet.wSize > 0)
		{
			...
			CPythonBattlePass::instance().UpdateBattlePass(info);
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_RefreshBattlePass", Py_BuildValue("()"));
			packet.wSize -= sizeof(info);
		}
	} break;
	case BATTLEPASS_GC_SUB_UPDATE:
	{
		int iPacketSize = packet.wSize - sizeof(TPacketBattlePass);
		if (iPacketSize > 0)
		{
			TPacketSubBattlePassUpdate kSubPacket;
			if (!Recv(sizeof(TPacketSubBattlePassUpdate), &kSubPacket))
				return false;

			TBattlePass info;
			info.missionVnum = kSubPacket.missionVnum;
			info.progress = kSubPacket.progress;
			info.isActive = kSubPacket.isActive;
...
#ifdef ENABLE_TARGET_INFO_DROP
bool CPythonNetworkStream::SendTargetInfoPacket(uint32_t vid)
{
	if (!__CanActMainInstance())
		return false;

	CInstanceBase* victim = CPythonCharacterManager::Instance().GetInstancePtr(vid);
	if (!victim)
		return false;

	const auto right_target = victim->IsEnemy() || victim->IsStone();
	if (!right_target)
		return false;

	SPacketCGTargetInfo packet;
	packet.vid = vid;
...
bool CPythonNetworkStream::RecvTargetInfoDrop()
{
	SPacketGCTargetInfo packet;
	if (!Recv(sizeof(packet), &packet))
		return false;

	packet.size -= sizeof(SPacketGCTargetInfo);

	if (packet.size > 0)
	{
		const auto count = packet.size / sizeof(SItemInfoDrop);
		if (count > 0)
		{
			std::vector<SItemInfoDrop> vecItemInfoDrop(count);
			if (!Recv(packet.size, &vecItemInfoDrop[0]))
				return false;

			for (const auto& v : vecItemInfoDrop)
			{
				CItemData* pItemData = nullptr;
				if (!CItemManager::instance().GetItemDataPointer(v.vnum, &pItemData))
					continue;

				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_AddTargetMonsterDropInfo", Py_BuildValue("(iii)", packet.race_id, v.vnum, v.count));
			}
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_RefreshTargetMonsterDropInfo", Py_BuildValue("(i)", packet.race_id));
...
#endif
#ifdef ENABLE_RANKING_SYSTEM
bool CPythonNetworkStream::SendRequestRanking(BYTE bCategory) {
	TPacketCGRequestRanking pack;
	pack.bHeader = HEADER_CG_RANKING_SYSTEM;
...
bool CPythonNetworkStream::RecvRankingSystem() {
	TPacketGCRequestRanking Packet;
	if (!Recv(sizeof(Packet), &Packet)) {
		Tracen("RecvRankingSystem Error");
		return false;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME],
		"BINARY_ClearPlayerRanking",
		Py_BuildValue("()"));

	for (BYTE i = 0; i < Packet.bCount; ++i)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_AddPlayerRanking",
			Py_BuildValue("(issiLi)", Packet.ranks[i].rank, Packet.ranks[i].name,
				Packet.ranks[i].guild, Packet.ranks[i].empire,
				Packet.ranks[i].count, Packet.ranks[i].selfPos));
	}
	return true;
}
#endif
