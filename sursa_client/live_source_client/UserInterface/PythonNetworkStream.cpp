...
class CMainPacketHeaderMap : public CNetworkPacketHeaderMap
{
	...
	public:
		CMainPacketHeaderMap()
		{
			...
			Set(HEADER_GC_RANKING_SYSTEM, TPacketType(sizeof(TPacketGCRequestRanking), STATIC_SIZE_PACKET));
#endif
			...
			Set(HEADER_GC_BATTLE_PASS, TPacketType(sizeof(TPacketBattlePass), DYNAMIC_SIZE_PACKET));
#endif
#ifdef INGAME_WIKI
			...
			Set(HEADER_GC_SEND_BIOLOG_INFO, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCSendInfoBiolog), STATIC_SIZE_PACKET));
#endif
...
#endif
			Set(HEADER_GC_REWARD_HOURS, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketRewardHours), STATIC_SIZE_PACKET));
			...
			Set(HEADER_GC_TARGET_INFO_DROP, TPacketType(sizeof(SPacketGCTargetInfo), DYNAMIC_SIZE_PACKET));
#endif
		}
};
...
bool CPythonNetworkStream::ConnectGameServer(UINT iChrSlot)
{
	...
	return CNetworkStream::Connect((DWORD)rkSimplePlayerInfo.lAddr, rkSimplePlayerInfo.wPort);
}
...
#else
	if (PacketType.iPacketType == CMainPacketHeaderMap::DYNAMIC_SIZE_PACKET)
#endif
	{
...
#ifdef __ENABLE_LARGE_DYNAMIC_PACKET__
	else if (PacketType.iPacketType == CMainPacketHeaderMap::LARGE_DYNAMIC_SIZE_PACKET) {
