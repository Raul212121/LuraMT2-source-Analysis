...
/*
	...
	if ((pkInstPicked->IsPet() || pkInstPicked->IsMount()) && pkInstPicked->IsAffect(CInstanceBase::AFFECT_INVISIBILITY))
		return false;
#endif
*/
...
const TItemData * CPythonPlayer::GetItemData(TItemPos Cell) const
{
	...
	case POTIONS_INVENTORY:
		return &m_playerStatus.aPotItem[Cell.cell];
	case BONUS_INVENTORY:
		return &m_playerStatus.aBnsItem[Cell.cell];
	case CHEST_INVENTORY:
		return &m_playerStatus.aSCSItem[Cell.cell];
#endif
...
void CPythonPlayer::SetItemData(TItemPos Cell, const TItemData & c_rkItemInst)
{
	...
	case UPGRADE_INVENTORY:
		...
		break;
	case POTIONS_INVENTORY:
		m_playerStatus.aPotItem[Cell.cell] = c_rkItemInst;
		break;
	case BONUS_INVENTORY:
		m_playerStatus.aBnsItem[Cell.cell] = c_rkItemInst;
		break;
	case CHEST_INVENTORY:
		m_playerStatus.aSCSItem[Cell.cell] = c_rkItemInst;
		break;
#endif
#ifdef ENABLE_SWITCHBOT
	case SWITCHBOT:
...
DWORD CPythonPlayer::GetItemCountByVnum(DWORD dwVnum)
{
	...
	for (int i = 0; i < c_Special_ItemSlot_Count; ++i)
	{
		const TItemData& c_rItemData = m_playerStatus.aPotItem[i];
		if (c_rItemData.vnum == dwVnum)
		{
			dwCount += c_rItemData.count;
		}
	...
	for (int i = 0; i < c_Special_ItemSlot_Count; ++i)
	{
		const TItemData& c_rItemData = m_playerStatus.aBnsItem[i];
		if (c_rItemData.vnum == dwVnum)
		{
			dwCount += c_rItemData.count;
		}
	...
	for (int i = 0; i < c_Special_ItemSlot_Count; ++i)
	{
		const TItemData& c_rItemData = m_playerStatus.aSCSItem[i];
		if (c_rItemData.vnum == dwVnum)
		{
			dwCount += c_rItemData.count;
		}
	...
	if (strlen(c_szOwnerName) > 0) {
		if (0 != strcmp(c_szOwnerName, GetName())) {
			...
			if (!CItemManager::Instance().GetItemDataPointer(
				...
				&pItemData)) {
				Tracenf(
					"CPythonPlayer::SendClickItemPacket(dwIID=%d) : Non-exist "
					"item.",
					dwIID);
				return;
			}
			...
			if (!IsPartyMemberByName(c_szOwnerName) ||
				...
				DWORD dwCurentTime = ELTimer_GetMSec();
				if (dwCurentTime >= m_DelayValue) {
					PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotPickItem",
						Py_BuildValue("()"));
					m_DelayValue = dwCurentTime + 1000;
				}
				return;
			}
		}
	...
	if (dwCurTime >= s_dwNextTCPTime) {
		...
		if (strlen(c_szOwnerName) > 0)
			if (0 != strcmp(c_szOwnerName, GetName())) {
				CItemData* pItemData;
				if (!CItemManager::Instance().GetItemDataPointer(
					CPythonItem::Instance().GetVirtualNumberOfGroundItem(
						dwIID),
					&pItemData)) {
					Tracenf(
						"CPythonPlayer::SendClickItemPacket(dwIID=%d) : "
						"Non-exist item.",
						dwIID);
					return;
				}
