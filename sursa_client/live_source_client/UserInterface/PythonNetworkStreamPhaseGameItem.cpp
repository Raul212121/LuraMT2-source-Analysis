...
bool CPythonNetworkStream::RecvItemUpdatePacket()
{
	...
	if (packet_item_update.count > rkPlayer.GetItemCount(packet_item_update.Cell)){
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Highlight_Item", Py_BuildValue("(ii)",
			packet_item_update.Cell.window_type, packet_item_update.Cell.cell));
	}
#endif
