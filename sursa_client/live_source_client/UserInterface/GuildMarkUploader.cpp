...
bool CGuildMarkUploader::Connect(const CNetworkAddress& c_rkNetAddr, DWORD dwHandle, DWORD dwRandomKey, DWORD dwGuildID, DWORD dwPlayerID, const char* c_szFileName, UINT* peError)
{
	__OfflineState_Set();
	SetRecvBufferSize(1024);
	SetSendBufferSize(1024);
...
void CGuildMarkUploader::SetSymbolAuthData(const TPacketGuildSymbolAuth* auth)
{
	std::memcpy(&m_symbol_auth, auth, sizeof(m_symbol_auth));
}
...
bool CGuildMarkUploader::__SendMarkPacket()
{
	...
	kPacketMarkUpload.pid = m_dwPlayerID;
	...
	if (!Send(sizeof(m_symbol_auth), &m_symbol_auth))
		return false;
...
bool CGuildMarkUploader::__LoginState_RecvPhase()
{
	...
	if (kPacketPhase.phase == PHASE_GUILD_MARK)
	{
...
bool CGuildMarkUploader::__LoginState_RecvHandshake()
{
...

	{
		...
		kPacketMarkLogin.dwHandshake = kPacketHandshake.dwHandshake;
