...
void CPythonPlayer::PickCloseMoney() {
	...
	if (CPythonSystem::instance().GetPickupMode() ==
		CPythonSystem::PICKUP_MODE_ALL) {
		std::vector<DWORD> vecCloseItems =
			rkItem.GetCloseMoneys(kPPosMain, __GetPickableDistance());
		...
		DWORD dwItemID;
		if (rkItem.GetCloseMoney(kPPosMain, &dwItemID,
			__GetPickableDistance())) {
			return;
		}
...
void CPythonPlayer::PickCloseItem() {
	...
	if (CPythonSystem::instance().GetPickupMode() ==
		CPythonSystem::PICKUP_MODE_ALL) {
		std::vector<DWORD> vecCloseItems =
			rkItem.GetCloseItems(kPPosMain, __GetPickableDistance());
		for (DWORD dwItemID : vecCloseItems) {
			SendClickItemPacket(dwItemID);
		}
	}
		...
		DWORD dwItemID;
		if (!rkItem.GetCloseItem(kPPosMain, &dwItemID,
			__GetPickableDistance())) {
			return;
		}
...
//		return;
	...
	// PyCallClassMemberFunc(m_ppyGameWindow, "SetPCTargetBoard", Py_BuildValue("(isiii)", pkInstTarget->GetVirtualID(), pkInstTarget->GetNameString()));
}
...
/*
	...
	if ((pkInstVictim->IsPet() || pkInstVictim->IsMount()) && pkInstVictim->IsAffect(CInstanceBase::AFFECT_INVISIBILITY))
	{
		__OnClickSmart(rkInstMain, isAuto, true);
		return;
	}
#endif
*/
...
/*
	...
	if ((pkInstVictim->IsPet() || pkInstVictim->IsMount()) && pkInstVictim->IsAffect(CInstanceBase::AFFECT_INVISIBILITY))
	{
		__OnPressSmart(rkInstMain, isAuto, true);
		return;
	}
#endif
*/
