...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		static void DiscordRunCallbacks();
#endif

		void SetDiscordUserId(uint64_t userId);
		...
		bool ConnectGameServer(UINT iChrSlot);
		...
		void SendOfflineshopBuyItem(DWORD dwOwnerID, DWORD dwItemID, bool isSearch, const offlineshop::TValutesInfo& valutes);
		...
		bool SendTargetInfoPacket(uint32_t vid);
#endif
		// Refine
...
#ifdef ENABLE_RANKING_SYSTEM
		bool SendRequestRanking(BYTE bCategory);
#endif
		...
		bool    SendChestDropInfo(DWORD itemvnum);
...
#ifdef ENABLE_SWITCHBOT
		void __RefreshSwitchbootWindow();
#endif
