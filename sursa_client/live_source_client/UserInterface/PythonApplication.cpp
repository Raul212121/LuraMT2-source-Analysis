...
bool CPythonApplication::Process()
{
	...
	CPythonNetworkStream::DiscordRunCallbacks();
#endif
...
bool LoadLocaleData(const char* localePath)
{
	...
	char szBattlePass[256];
	snprintf(szBattlePass, sizeof(szBattlePass), "%s/battlepass.json", localePath);
	if (!CPythonBattlePass::Instance().Initialize(szBattlePass))
		TraceError("LoadLocaleData: error while loading %s.", szBattlePass);
	...
	{
		TraceError("LoadLocaleData - LoadInsultList(%s) Error", szInsultList);
		return false;
	}
	*/
