...
bool CPythonNetworkStream::RecvRewardHours()
{
	TPacketRewardHours packet;
	if (!Recv(sizeof(TPacketRewardHours), &packet))
	{
		Tracen("CPythonNetworkStream::RecvRewardHours - PACKET READ ERROR");
		return false;
	}

	CPythonPlayer::instance().SetHoursStage(packet.stage);
	return true;
}
