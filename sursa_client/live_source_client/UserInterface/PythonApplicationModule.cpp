...
PyObject* appIsExistFile(PyObject* poSelf, PyObject* poArgs)
{
	...
	bool isExist=FileSystem::FileSystemManager::Instance().DoesFileExist(szFileName);
...
class CTextLineLoader
{
	public:
		CTextLineLoader(const char * c_szFileName)
		{
			...
			m_kTextFileLoader.Bind(file->get_size(), file->get_data());
		}
...
void initapp()
{
	...
	PyModule_AddIntConstant(poModule, "ENABLE_TARGET_INFO_DROP",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_TARGET_INFO_DROP",	0);
#endif
...
// #endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_ANTIEXP_RING", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_ANTIEXP_RING", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_SPECIAL_INVENTORY_SYSTEM", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SPECIAL_INVENTORY_SYSTEM", 0);
#endif

#ifdef ENABLE_MOUSEWHEEL_INPUT
	PyModule_AddIntConstant(poModule, "ENABLE_MOUSEWHEEL_INPUT", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MOUSEWHEEL_INPUT", 0);
#endif

#ifdef ENABLE_NEW_GAME_OPTIONS
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_GAME_OPTIONS", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_GAME_OPTIONS", 0);
#endif
...
#if defined(ENABLE_MAINTENANCE_SYSTEM)
	PyModule_AddIntConstant(poModule, "ENABLE_MAINTENANCE_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MAINTENANCE_SYSTEM", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_EXTEND_ITEMS_STACK",	1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_EXTEND_ITEMS_STACK",	0);
#endif
...
#ifdef ENABLE_COSTUME_EXTENDED_RECHARGE
	PyModule_AddIntConstant(poModule, "ENABLE_COSTUME_EXTENDED_RECHARGE", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_COSTUME_EXTENDED_RECHARGE", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_EASTER_EVENT", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_EASTER_EVENT", 0);
#endif

#ifdef ENABLE_PET_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_PET_COSTUME_SYSTEM", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_PET_COSTUME_SYSTEM", 0);
#endif

#ifdef ENABLE_PERMANENT_AFFECT
	PyModule_AddIntConstant(poModule, "ENABLE_PERMANENT_AFFECT", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_PERMANENT_AFFECT", 0);
#endif
...
#if defined(ENABLE_STONE_ON_MINIMAP)
	PyModule_AddIntConstant(poModule, "ENABLE_STONE_ON_MINIMAP", 1);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_STONE_ON_MINIMAP", 0);
#endif
