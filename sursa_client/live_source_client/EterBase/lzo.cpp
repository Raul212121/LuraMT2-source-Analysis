...
bool CLZO::CompressEncryptedMemory(CLZObject & rObj, const void * pIn, UINT uiInLen, int type, DWORD * pdwKey)
{
    rObj.BeginCompress(pIn, uiInLen);
	
    if (rObj.Compress()) {
		if (rObj.Encrypt(pdwKey)) return true;
		return false;
    }   
...
bool CLZO::Decompress(CLZObject & rObj, const BYTE * pbBuf, int type, DWORD * pdwKey)
{
    ...
    if (!rObj.Decompress(pdwKey)) return false;
