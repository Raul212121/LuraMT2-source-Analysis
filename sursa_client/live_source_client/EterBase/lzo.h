...
class CLZO : public CSingleton<CLZO>
{
	public:
		...
		bool	CompressEncryptedMemory(CLZObject & rObj, const void * pIn, UINT uiInLen, int type, DWORD * pdwKey);
		bool	Decompress(CLZObject & rObj, const BYTE * pbBuf, int type, DWORD * pdwKey = NULL);
		BYTE *	GetWorkMemory();
