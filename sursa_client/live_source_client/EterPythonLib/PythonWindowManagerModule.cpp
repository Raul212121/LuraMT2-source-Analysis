...
PyObject* wndSetSkipItemCheck(PyObject* poSelf, PyObject* poArgs)
{
	UI::CWindow* pWin;
	if (!PyTuple_GetWindow(poArgs, 0, &pWin))
		return Py_BuildException();
	...
	int info;
	if (!PyTuple_GetInteger(poArgs, 2, &info))
		return Py_BuildException();

	UI::CSlotWindow* pSlotWin = (UI::CSlotWindow*)pWin;
	//pSlotWin->SkipItemCheck(iSlotIndex, info);
