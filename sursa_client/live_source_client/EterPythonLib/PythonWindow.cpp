...
namespace UI
{
	...
	{
		m_SetScale = std::bind(&CGraphicExpandedImageInstance::SetScale, std::placeholders::_1, fScaleX, fScaleY);
		std::for_each(m_ImageVector.begin(), m_ImageVector.end(), m_SetScale);
	}
	...
	void CButton::SetUp()
	{
		SetCurrentVisual(&m_upVisual);
		m_isPressed = FALSE;
		PyCallClassMemberFunc(m_poHandler, "OnMouseOverOut", BuildEmptyTuple()); // used for mouse events
	}
	void CButton::Up()
	{
		if (IsIn())
	...
	{
		SetCurrentVisual(&m_overVisual);
		PyCallClassMemberFunc(m_poHandler, "OnMouseOverIn", BuildEmptyTuple()); // used for mouse events
	}
	void CButton::Down()
	{
		m_isPressed = TRUE;
