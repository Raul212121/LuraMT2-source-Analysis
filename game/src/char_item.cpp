	...
	if (item->GetVnum() == 76020)
	{
		...
		if (pPC)
		{
			...
			if (iTimeWait > 1)
			{
				...
				ChatPacket(CHAT_TYPE_INFO, "Timpul A fost restat!");				
			}
		}
	}
							...
							case 93100:
									{
										...
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Vous avez deja completer ce Pass de combat."));
											return false;
										}
										...
										if(iSeconds < 0)
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Erreur systeme, veuillez contacter un administrateur."));
											return false;
										}
										
										if(FindAffect(AFFECT_BATTLE_PASS))
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Vous avez deja le Pass de combat d'activer sur ce compte."));
											return false;
										}
										else
										{
									...
									if (bBattlePassId)
									{
										...
										if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassId, COLLECT_ITEM, &dwItemVnum, &dwCount))
										{
											if (dwItemVnum == item->GetVnum() && GetMissionProgress(COLLECT_ITEM, bBattlePassId) < dwCount)
												UpdateMissionProgress(COLLECT_ITEM, bBattlePassId, bCount, dwCount);
										}
									}
								}
#endif
...
#endif		
							BYTE bCount = MIN(200 - item2->GetCount(), bCount);
							bCount -= bCount;

							item2->SetCount(item2->GetCount() + bCount);
	...
	if (p->dwFlags & ITEM_FLAG_STACKABLE && p->bType != ITEM_BLEND) 
	{
		for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
		{
			...
			if (item->GetVnum() == dwItemVnum && FN_check_item_socket(item))
			{
				...
				BYTE bCount = MIN(200 - item->GetCount(), bCount);
				bCount -= bCount;
