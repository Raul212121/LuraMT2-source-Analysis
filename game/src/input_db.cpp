...
void CInputDB::BattlePassLoad(LPDESC d, const char* c_pData)
{
	if (!d)
	{
		sys_err("BATTLE_PASS_LOAD: desc null");
		return;
	}

	if (!d->GetCharacter())
	{
		sys_err("BATTLE_PASS_LOAD: character null for handle %u", d->GetHandle());
		return;
	}
	...
	if (!ch)
	{
		sys_err("BATTLE_PASS_LOAD: ch null for handle %u", d->GetHandle());
		return;
	}
	...
	sys_err("BATTLE_PASS_LOAD: incoming pid %u, char pid %u, count %u", dwPID, ch->GetPlayerID(), dwCount);

	if (ch->GetPlayerID() != dwPID)
	{
		sys_err("BATTLE_PASS_LOAD: pid mismatch incoming %u char %u", dwPID, ch->GetPlayerID());
		return;
	}

	ch->LoadBattlePass(dwCount, (TPlayerBattlePassMission*)c_pData);
	sys_err("BATTLE_PASS_LOAD: loaded for pid %u count %u", dwPID, dwCount);
}
...
void CInputDB::BattlePassLoadRanking(LPDESC d, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Nu exista clasamente inregistrate in prezent!"));
	}
}
#endif
