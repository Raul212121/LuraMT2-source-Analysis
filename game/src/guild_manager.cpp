...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
    ...
    {
        gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<e∏∏Î‚ÄsLì> e∏∏Î‚ÄsLì et¥Î¶‚Ä?et¥ e ÅÌ‚Ä?©Ì‚Ä?Àseß‚Ç¨ e‚Ä?L eL µÎ‚ÄaÀÜÎ‚Äa§."));
        return 0;
    }
    ...
    if (pmsg->Get()->uiNumRows > 0)
    {
        ...
        if (!(row[0] && row[0][0] == '0'))
        {
            gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<e∏∏Î‚ÄsLì> et¥ÎZ∏ e∞‚Ñ?et‚Ç¨ et¥Î¶‚Ä?etÀs e∏∏Î‚ÄsLìe∞‚Ç¨ e?ÀÜeL µÎ‚ÄaÀÜÎ‚Äa§."));
            return 0;
        }
    }
    else
    {
        gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<e∏∏Î‚ÄsLì> e∏∏Î‚ÄsLìÎAL eCíte‚Ä?±Ì‚Ä?  eÀÜÀs e‚Äî‚Ä eL µÎ‚ÄaÀÜÎ‚Äa§."));
        return 0;
    }
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
		...
		g2->DeclareWar(guild_id1, bType, GUILD_WAR_RECV_DECLARE))
	{
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), LC_TEXT("%s ±ÊµÂ∞° %s ±ÊµÂø° º±¿¸∆˜∞Ì∏¶ «œø¥Ω¿¥œ¥Ÿ!"), TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
			SendNotice(buf);
		}
	}
}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> %s ±ÊµÂ∞° ±ÊµÂ¿¸¿ª ∞≈∫Œ«œø¥Ω¿¥œ¥Ÿ."), g1->GetName());
	}
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), LC_TEXT("%s ±ÊµÂøÕ %s ±ÊµÂ∞° ¿·Ω√ »ƒ ¿¸¿Ô¿ª Ω√¿€«’¥œ¥Ÿ!"), g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::StartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	char buf[256];
	snprintf(buf, sizeof(buf), LC_TEXT("%s ±ÊµÂøÕ %s ±ÊµÂ∞° ¿¸¿Ô¿ª Ω√¿€«œø¥Ω¿¥œ¥Ÿ!"), g1->GetName(), g2->GetName());
	SendNotice(buf);
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		...
		if (bDraw)
		{
			snprintf(buf, sizeof(buf), LC_TEXT("%s ±ÊµÂøÕ %s ±ÊµÂ ªÁ¿Ã¿« ¿¸¿Ô¿Ã π´Ω¬∫Œ∑Œ ≥°≥µΩ¿¥œ¥Ÿ."), g1->GetName(), g2->GetName());
		}
		else
		{
			if ( g1->GetWarScoreAgainstTo( g2->GetID() ) > g2->GetWarScoreAgainstTo( g1->GetID() ) )
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%s ±ÊµÂ∞° %s ±ÊµÂøÕ¿« ¿¸¿Ôø°º≠ Ω¬∏Æ «ﬂΩ¿¥œ¥Ÿ."), g1->GetName(), g2->GetName());
			}
			else
			{
				snprintf(buf, sizeof(buf), LC_TEXT("%s ±ÊµÂ∞° %s ±ÊµÂøÕ¿« ¿¸¿Ôø°º≠ Ω¬∏Æ «ﬂΩ¿¥œ¥Ÿ."), g2->GetName(), g1->GetName());
			}
		}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ±ÊµÂ¿¸¿Ã √Îº“ µ«æ˙Ω¿¥œ¥Ÿ."));
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ±ÊµÂ¿¸¿Ã √Îº“ µ«æ˙Ω¿¥œ¥Ÿ."));
	}

	if (g1 && g2)
	{
		char buf[256+1];
		snprintf(buf, sizeof(buf), LC_TEXT("%s ±ÊµÂøÕ %s ±ÊµÂ ªÁ¿Ã¿« ¿¸¿Ô¿Ã √Îº“µ«æ˙Ω¿¥œ¥Ÿ."), g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
