...
bool CBattlePass::ReadBattlePassGroup()
{
	...
	if (NULL == pGroupNode)
	{
		sys_err("battle_pass.txt necesita un bilet pentru battlepass.");
		return false;
	}
	...
	if (0 == n)
	{
		sys_err("Battlepass-ul pentru grup este gol!");
		return false;
	}
...
void CBattlePass::BattlePassRequestOpen(LPCHARACTER pkChar)
{
	...
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Ceva nu a mers bine. Va rugam să incercati din nou mai tarziu."));
		return;
	}
	...
	if (!bBattlePassId)
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu ai un bilet BattlePass activat."));
		return;
	}
	...
	{
		if (test_server)
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu exista BattlePass cu acest ID. (%d)"), bBattlePassId);
	...
	{
		if (test_server)
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu exista niciun BattlePass cu acest nume. (%s)"), battlePassName.c_str());
	...
	{
		if (test_server)
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu exista recompensa pentru acest BattlePass. (%s)"), battlePassName.c_str());
...
void CBattlePass::BattlePassRewardMission(LPCHARACTER pkChar, DWORD bMissionType, DWORD bBattlePassId)
{
	...
	if (!bBattlePassId)
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu ai un BattlePass activat."));
		return;
	}
...
void CBattlePass::BattlePassRequestReward(LPCHARACTER pkChar)
{
	...
	{
		char buf[256] = { 0 };
		snprintf(buf, sizeof(buf), LC_TEXT("Jucatorul %s a finalizat cu succes BattlePass %s."), pkChar->GetName(), LC_TEXT(battlePassName.c_str()));
		BroadcastNotice(buf);
...
void CBattlePass::BattlePassReward(LPCHARACTER pkChar)
{
	...
	{
		if (test_server)
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu exista un BattlePass cu acest ID. (%d)"), bBattlePassId);
